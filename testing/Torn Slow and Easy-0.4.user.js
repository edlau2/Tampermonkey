...
(function() {
    ...
    function updateStats(data) {
        ...
        if (data.won == 1) {
            wins++;
            lastResultWon = true;
            moneyWon += parseInt(data.moneyWon);
        } else {
            losses++;
        ...
        $.ajax = function (options) {
            ...
            if (options.data != null && options.data.sid == 'slotsData' && options.data.step == 'play') {
                ...
                options.success = function (data, textStatus, jqXHR) {
                    log(ts(), " Success, textStatus: ", textStatus);
                    log(ts(), " Success, data: ", data);
            ...
            <div id="stats-tbl">
                <table><tbody>
                    ...
                    <tr>
                        ...
                        <td><input id="xrst" class="xedx-torn-btn" value="Reset"></td>
                    </tr>
                </tbody></table>
            </div>
        `;
    ...
    */
    ...
    function placeBet(btn) {
        ...
        log(ts(), ' Placing bet for ' + asCurrency(btn.getAttribute('data-bet')) + ' in ' + ms + 'ms');
    ...
    function handleGrvTimeout() {
        ...
        log(ts(), ' Payout: ' + output);
        log(ts(), ' handleGrvTimeout: might bet now!');
