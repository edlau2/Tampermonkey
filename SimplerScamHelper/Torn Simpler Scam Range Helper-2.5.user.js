...
(function() {
    ...
    var closeResultDelayMs = GM_getValue("closeResultDelayMs", 5000);          // Delay in ms (3 seconds default)
    ...
    const isXedx = GM_getValue("isXedx", false);
    ...
    function getSkill() {
        ...
        log("updateSkill, get, ", stat, " tmp: ", tmp, " curr: ", dblCurrSkill, " prev: ", prevCurrSkill);
    ...
    const defMoveArray60 =
          { name: "idxMoveArray60", 0: [[6, 12], [2, 5], [-4, -1]], 1: [[9, 18], [3, 7], [-6, -2]], 2: [[11, 21], [4, 8], [-4, -7]],
    ...
    function closeResultPanel(btn, target) {
        if ($(btn).length == 0) return log("Invalid close btn!");

        $(btn).css("border", "1px solid green");

        log("Preparing to close result...");
        $(target).animate({
                opacity: 0,
            }, 2000, function() {
                $(btn)[0].click();
                log("Close button clicked! (animate done)");
            });
    ...
    function handleCapitalize(retries=0) {
        ...
        //let entered = $(currVirtItemDiv).find("[class*='outcome_'][class*='entered_']");
        let entered = $(currVirtItemDiv).find("[class*='entered_']");
        //$(entered).css("border", "1px solid limegreen");
        //log("capitalize: entered: ", $(entered));

        let btn = $(entered).find("button[class^='closeButton_']");
        $(btn).css("border", "1px solid red");
        //log("capitalize: btn: ", $(btn));
        ...
        // $(virt).find("button[class^='closeButton_']") may be 2!
        // $(currVirtItemDiv).find("[class*='outcome_'][class*='entered_']") is parent way up
        // $(virt).find("[class*='outcome_'][class*='entered_']").find("button[class^='closeButton_']") do in 2 steps?

    }

    // ================================== Handle a 'commit' btn click =============================
    // Read, Respond, ...do I get this first, THEN goes to torn? If so, grab skill first
    // to check for change....
    var skillAtCommit;
