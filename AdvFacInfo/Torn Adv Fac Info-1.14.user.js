...
(function() {
    ...
    var apiFrequencyHrs = GM_getValue("apiFrequencyHrs", 6);
    ...
    function getAllKeysFromObjectStore(db, storeName=facStatsStore) {
    ...
    // TBD....
    function fillCtrlPanelRows() {
        const transaction = facDb.transaction([facStatsStore], 'readonly'); // Create a read-only transaction
        const objectStore = transaction.objectStore(facStatsStore);

        const cursorRequest = objectStore.openCursor();
        const data = []; // Array to store retrieved key-value pairs
        cursorRequest.onsuccess = (event) => {
            const cursor = event.target.result;
            if (cursor) {
                const key = cursor.key;       // member ID
                const value = cursor.value;   // Object entry
                data.push({ key, value });

                // Move to the next record
                cursor.continue();
            } else {
                // No more records, cursor is undefined
                console.log('All data retrieved:', data);
            }
        };

        cursorRequest.onerror = (event) => {
            log('Error opening cursor:', event.target.error);
        };
    ...
    function addTblHdr() {
        let hdr = `<tr><th>Member Name</th><th>Last Update</th><th>View Details</th><th>Remove</th></tr>`;
        $("#db-table thead.sticky-thead").append(hdr);
    }

    function fillDbTblRows(retries = 0) {
        debug("Fill DB table: ", $("#db-table tbody").length, Object.keys(membersList).length);
        ...
        for (const [key, value] of Object.entries(membersList)) {
            let row = `<tr class='db-row'>
                          <td><span>${value.name} [${key}]</span></td>
                          <td><span>Expandable Data</span></td>
                      </tr>`;
    ...
    function addTableSort(parentId) {
        ...
        function TableComparer(index, dir) {
            return function (a, b) {
                ...
                //let result = (dir == 'asc') ? val_a - val_b : val_b - val_a;
