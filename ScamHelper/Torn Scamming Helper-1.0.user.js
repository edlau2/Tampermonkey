...
(function() {
    ...
    function getMethodAndLevel(target) {
        ...
        // Use indexOf...
        let level = -1;
        if (method == "delivery" || method == "family" || method == "prize")
            level = 0;

        // charity, tech support
        if (method == "charity" || method.indexOf('tech') > -1)
            level = 20;

        // vacation, tax

        let methodAndLevel = {method: method, level: level};
        log("methodAndLevel: ", methodAndLevel);
    ...
    function applyCellHighlights(btn, accelMod, cl, retry) {
        ...
        // Need to get correct array dependent on level of crime.

        let rc = getMethodAndLevel(btn);
        log("[applyCellHighlights] method/level: ", rc);

        let useArray = idxMoveArray;
        if (rc.level == 20) useArray = idxMoveArray20;
        if (rc.level == 40) useArray = idxMoveArray40;

        let entry = useArray[parseInt(accel)][parseInt(btnIdx)];
