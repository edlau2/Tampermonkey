...
function versionCheck(silent=false) {
    ...
    if (parseFloat(GM_info.script.version) > parseFloat(curr_ver)) {
...
// --------------------
function xedx_TornRacingQueryv2(ID, selection, callback, param=null) {
    xedx_TornGenericQueryv2('racing', ID, selection, callback, param);
}
...
// Helper to converta style string into a JSON representation of the CSS
function styleToJson(styleStr) {
    let parts = (styleStr && styleStr.length) ? styleStr.split(";") : null;
    let ret = {};
    ...
    if (parts && parts.length) {
        parts.forEach(entry => {
            if (entry) {
                ...
                ret[keyVal[0].toString().trim()] = keyVal[1].toString().trim();
            }
        });
    }
