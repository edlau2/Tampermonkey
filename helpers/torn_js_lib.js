...
// @description Common Torn JS functions
// @author      me
...
var api_key = GM_getValue('gm_api_key', undefined);
...
const getApiKey = () => { return api_key; }

const getPlayerId = () => {
    const user = JSON.parse($("#torn-user").val());
...
function checkCloudFlare(clickIt) {
    const active = document.querySelector("#challenge-form") ||
        location.href.indexOf('recaptcha') > -1 ||
        document.querySelector(".iAmUnderAttack");
...
const isIndexPage = () => {return (location.href.indexOf("index.php") > -1)}
const isItemPage = () => {return (location.href.indexOf("item.php") > -1)}
const isFactionPage = () => {return (location.href.indexOf("factions.php") > -1)}
const isGymPage = () => {return (location.href.indexOf("gym.php") > -1)}
const isAttackPage = () => {return (location.href.indexOf("loader.php?sid=attack&user2ID") > -1)}
const isStocksPage = () => {return (location.href.indexOf("page.php?sid=stocks") > -1)}
const isRacePage = () => {return (location.href.indexOf("loader.php?sid=racing") > -1)}
const isBazaarPage = () => {return (location.href.indexOf("bazaar.php") > -1)}
const isHospPage = () => {return (location.href.indexOf("hospital.php") > -1)}
const isJailPage = () => {return (location.href.indexOf("jailview.php") > -1)}
const isPointsPage = () => {return (location.href.indexOf("page.php?sid=points") > -1)} //https://www.torn.com/page.php?sid=points
const isUserListPage = () => {return (location.href.toLowerCase().indexOf("userlist") > -1)}
const isAmmoPage = () => {return (location.href.toLowerCase().indexOf("sid=ammo") > -1)}
const isModsPage = () => {return (location.href.toLowerCase().indexOf("sid=itemsmods") > -1)};
const isJobsPage = () => {return (location.href.toLowerCase().indexOf("joblist") > -1)};
const isTravelPage = () => {return (location.href.toLowerCase().indexOf("travelagency") > -1)};
const isApiPage = () => {return (location.href.toLowerCase().indexOf("api.htm") > -1)};
const isCrimePage = () => {return (location.href.indexOf("sid=crimes") > -1)}

// Can't call from "run-at: start" with no jquery or top page headers...
const isHomePage = () => {
    const myDiv = document.getElementById('skip-to-content');
    const hdrTxt = myDiv.textContent
...
// Return the page sid for current page. For example, an attack log is at
