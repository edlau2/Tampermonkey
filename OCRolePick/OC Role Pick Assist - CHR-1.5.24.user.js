...
(async function() {
    ...
    // https://github.com/edlau2/Tampermonkey/raw/refs/heads/master/OCRolePick/OC%20Role%20Pick%20Assist%20-%20CHR.user.js
    ...
    GM_setValue("uploadMyCprs", uploadMyCprs);
    GM_setValue("collectAllCprs", collectAllCprs);
    ...
    var ocRoles = [
     ...
     const cprsTemplate = {
        "Blast from the Past": { "PICKLOCK #1": 0, "HACKER": 0, "ENGINEER": 0, "BOMBER": 0, "MUSCLE": 0, "PICKLOCK #2": 0 },
        "Stacking the Deck": { "HACKER": 0, "IMPERSONATOR": 0, "CAT BURGLAR": 0, "DRIVER": 0 },
        "Ace in the Hole": { "HACKER": 0, "DRIVER": 0, "MUSCLE #1": 0, "IMPERSONATOR": 0, "MUSCLE #2": 0 },
        "Break the Bank": { "ROBBER": 0, "MUSCLE #1": 0, "THIEF #1": 0, "MUSCLE #2": 0, "MUSCLE #3": 0, "THIEF #2": 0 },
     };

    var userCprs = JSON.parse(GM_getValue("userCprs", JSON.stringify({})));

    var myCprs = JSON.parse(GM_getValue("myCprs", JSON.stringify({})));
    if (Object.keys(myCprs).length == 0) {
        GM_setValue("myCprs", JSON.stringify(cprsTemplate));
        myCprs = JSON.parse(GM_getValue("myCprs", JSON.stringify({})));
    }

    function getCiValue(obj, searchKey) {
        const lowerSearchKey = searchKey.toLowerCase();
        const foundKey = Object.keys(obj).find(key => key.toLowerCase() === lowerSearchKey);
       return foundKey ? obj[foundKey] : undefined;
    }

    log("Loaded myCprs: ", JSON.stringify(myCprs, null, 4));
                ...
                if ((typeof entry.Positions) == 'object') {
                    for (const [pos, req] of Object.entries(entry.Positions)) {
                        if (currEntry.Positions[pos] == req) {
                            ...
                            debug("Changing ", entry.OCName, `: currEntry.Positions[${pos}]: `, currEntry.Positions[pos], " to ", req);
                    ...
                    if (currEntry.Positions == entry.Positions) {
                        ...
                        debug("Changing ", entry.OCName, " defaults from", currEntry.Positions, " to ", entry.Positions);
    ...
    function displayHtmlToolTipEx(node, text, options) {
        ...
        if (options.btnSelector && options.btnFn) {
            $(node).on( "tooltipopen", function( event, ui ) {
                let ttList = $(".ui-tooltip").not(ui.tooltip);
                $(ttList).each(function() {
                    let tooltipId = $(this).attr('id');
                    let originalElement = $('[aria-describedby="' + tooltipId + '"]');
                    $(originalElement).tooltip("close");
                });
    ...
    function handleToolTipBtn(e, node) {
        ...
        let dataStr = tmp.replace(/"/g, "").replaceAll('{', '').replaceAll('}', '').replace(/[\r\n]+/g, "").trim();
        ...
        // My cpr's (if known)
        if (myCprs[ocName]) {
            tmp = JSON.stringify(myCprs[ocName], null, 4);
            let dataStr2 = tmp.replace(/"/g, "").replaceAll('{', '').replaceAll('}', '').replace(/[\r\n]+/g, "").trim();
            let dataParts2 = dataStr2.split(',');
            dataStr2 = '\nYour CPRs for this OC:\n'; //'\n                  ';
            dataParts2.forEach(part => {
                if (part.indexOf(': 0') > -1) part = part.replace('0', '?');
                dataStr2 += ('\n                  ' + part.trim());
            });
        ...
        alert(`${ocData.OCName} requirements:\n${dataStr}`);
    }
    ...
    function processScenario(panel) {
        ...
        let saveMyCprs = false;
        let saveUserCprs = false;
        Array.from(slots).forEach(slot => {
            ...
            // Save success chance, can optionally share
            let myEntry = getCiValue(myCprs, ocName);
            if (myEntry) {
                if (myEntry[rawRole] != successChance && !userName) {
                    myEntry[rawRole] = successChance;
                    saveMyCprs = true;
                }
            }

            // Save other known ones also...
            if (collectAllCprs && userName && !isMobile) {
                let userEntry = getCiValue(userCprs, userName);
                if (!userEntry) {
                    userCprs[userName] = JSON.parse(JSON.stringify(cprsTemplate));
                    userEntry = userCprs[userName];
                }
                let ocEntry = userEntry[ocName];
                if (!ocEntry) {
                    ocEntry = getCiValue(userEntry, ocName);
                }
                if (parseInt(ocEntry[rawRole]) != parseInt(successChance)) {
                    ocEntry[rawRole] = successChance;
                    saveUserCprs = true;
                }
            }
        ...
        let entry = {id: parseInt(ocId), lvl: parseInt(level), slots: slots,
                     ...
                     complete: compPct/slotCount, sortScore: sortScore}; 
        ...
        if (saveMyCprs == true) {
            GM_setValue("myCprs", JSON.stringify(myCprs));
        }
        if (saveUserCprs == true) {
            GM_setValue("userCprs", JSON.stringify(userCprs));
            userCprs = JSON.parse(GM_getValue("userCprs", JSON.stringify({})));
        }
    ...
    function handleHelp(e) {
        alertWithTimeout(`Press 'Best' to scroll to the OC you should join.\n
                          ...
                          The '?' button will display the cpr requirements for all positions.`, 15);
    }

    function hookPageTabs(retries=0) {
        let tabBtns = $("#faction-crimes-root > div > div[class^='buttonsContainer_'] > button");
        if (!$(tabBtns).length) {
            if (retries++ < 50) return setTimeout(hookPageTabs, 250, retries);
            return log("[hookPageTabs] timed out.");
        }
        ...
        if (devMode == true && !isMobile) {
            let tab = getRecruitingTab();
            $(tab).on('contextmenu', function(e) {
                e.preventDefault();
                e.stopPropagation();

                let msg = "<h1>All known user CPRs</h1><br><p><pre><code>" +
                    JSON.stringify(userCprs, null, 4) + "</code></pre></p>";

                let newWindow = window.open("", "_blank");
                if (newWindow) {
                    $(newWindow.document.body).html(msg);
                    newWindow.document.title = "User CPRs";
                } else {
                    alert("User CPR tab blocked!");
                }
            });
        }
    }
    ...
    function installUI(retries=0) {
        ...
        $("#x-help-btn").on('click', handleHelp);
