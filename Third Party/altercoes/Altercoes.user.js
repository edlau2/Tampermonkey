...
GM_setValue('alt_versao','20211104.1');
...
var lactivo6 = GM_getValue('lactivo6', null);
var lactivo7 = GM_getValue('lactivo7', null);
...
var nomeL6 = GM_getValue('nomeL6', null);
var nomeL7 = GM_getValue('nomeL7', null);
...
var link6 = GM_getValue('link6', null);
var link7 = GM_getValue('link7', null);
...
var obookie  = GM_getValue('obookie', null);
...
function handler_mails(){
    if(this.readyState == 4 && this.status == 200) {
        if(this.responseText != null){

            var httpResp = this.responseText;
            var apiResponse = JSON.parse(httpResp);
...
if (document.location.href.match(/\/index\.php$/) || document.location.href.match(/\/authenticate\.php$/) || document.location.href.match(/\/city\.php$/) || document.location.href.match(/torn.com\/$/) || document.location.href.match(/\/sidebar.php$/)) {
  ...
  if( $("#skip-to-content").text().match(/Traveling/) || $("#skip-to-content").text().match(/Mexico|Canada|Cayman Islands|Hawaii|United Kingdom|Argentina|Switzerland|Japan|China|South Africa|Dubai/) ){
    //Get number of mails:
    requestAPI('//api.torn.com/user/?selections=notifications,money&key=', api_key, true, handler_mails);
  }
}
...
if(divProperties != null && lvault == '1'){
   ...
   addLink(divProperties, link, 'Vault', 1);
}
...
if(divCity != null){
  ...
  if(lraces == '1')
    addLink(divCity, 'racing.php', 'Races', 1);
  if(lstockmarket == '1')
    addLink(divCity, 'stockexchange.php', 'Stock Market', 1);
  if(useTravAg == '1')
    var travelAgLink = addLink(divCity, 'travelagency.php', 'Travel Agency', 1);
  if(lbounties == '1')
...
if(divCasino != null){
  if(lpoker == '1')
    //addLink(divCasino, 'pokerwindow.php', 'Poker', 1, '_blank');
    addLink(divCasino, 'loader.php?sid=holdem', 'Poker', 1, '_blank');
  if(lbookies == '1')
    addLink(divCasino, 'page.php?sid=bookie#/your-bets', 'Bookies', 1);
  if(lrroulette == '1')
...
function hide_icons(jNode){
    //find('[id*=anotherid]')
  if(osex == '1'){
    jNode.find('[id*=icon6-]').remove();
    jNode.find('[id*=icon7-]').remove();
  }
  if( odonator == '1'){
      jNode.find('[id*=icon3-]').remove();
      jNode.find('[id*=icon4-]').remove();
  }
  if( omarried == '1'){
      jNode.find('[id*=icon8-]').remove();
  }
  if( ojob == '1'){
      jNode.find('[id*=icon21-]').remove();
      jNode.find('[id*=icon22-]').remove();
      jNode.find('[id*=icon23-]').remove();
      jNode.find('[id*=icon24-]').remove();
      jNode.find('[id*=icon25-]').remove();
      jNode.find('[id*=icon26-]').remove();
      jNode.find('[id*=icon27-]').remove();
      jNode.find('[id*=icon73-]').remove();
  }
  if( ofaction == '1'){
      jNode.find('[id*=icon9-]').remove();
      jNode.find('[id*=icon74-]').remove();
  }
  if( oeduc == '1'){
      jNode.find('[id*=icon19-]').remove();
      jNode.find('[id*=icon20-]').remove();
  }
  if( obank == '1'){
      jNode.find('[id*=icon29-]').remove();
      jNode.find('[id*=icon30-]').remove();
      jNode.find('[id*=icon31-]').remove();
  }
  if( oloan == '1'){
      jNode.find('[id*=icon33-]').remove();
  }
  if( oitem == '1'){
      jNode.find('[id*=icon34-]').remove();
      jNode.find('[id*=icon35-]').remove();
      jNode.find('[id*=icon36-]').remove();
  }
  if( opoints == '1'){
      jNode.find('[id*=icon54-]').remove();
  }
  if( ostock == '1'){
      jNode.find('[id*=icon38-]').remove();
  }
  if( otrade == '1'){
      jNode.find('[id*=icon37-]').remove();
  }
  if( orace == '1'){
      jNode.find('[id*=icon17-]').remove();
      jNode.find('[id*=icon18-]').remove();
  }
  if( obounty == '1'){
      jNode.find('[id*=icon13-]').remove();
  }
  if( obookie == '1'){
      jNode.find('[id*=icon28-]').remove();
  }
  //Auction
  if( obooster == '1'){
      jNode.find('[id*=icon39-]').remove();
      jNode.find('[id*=icon40-]').remove();
      jNode.find('[id*=icon41-]').remove();
      jNode.find('[id*=icon42-]').remove();
      jNode.find('[id*=icon43-]').remove();
  }
  if( omedical == '1'){
      jNode.find('[id*=icon44-]').remove();
      jNode.find('[id*=icon45-]').remove();
      jNode.find('[id*=icon46-]').remove();
      jNode.find('[id*=icon47-]').remove();
      jNode.find('[id*=icon48-]').remove();
  }
  if( odrug == '1'){
      jNode.find('[id*=icon49-]').remove();
      jNode.find('[id*=icon50-]').remove();
      jNode.find('[id*=icon51-]').remove();
      jNode.find('[id*=icon52-]').remove();
      jNode.find('[id*=icon53-]').remove();
  }
  if(olevel100 == '1'){
      jNode.find('[id*=icon5-]').remove();
  }
  if( ohosp == '1'){
      jNode.find('[id*=icon15-]').remove();
  }
  if( ojail == '1'){
      jNode.find('[id*=icon16-]').remove();
  }
  if( ollife == '1'){
      jNode.find('[id*=icon12-]').remove();
  }
  //Drug Addiction
  //Radiation Sickness
  if( obook == '1' ){
      jNode.find('[id*=icon68-]').remove();
  }

  if( oupkeep == '1'){
      jNode.find('[id*=icon78-]').remove();
      jNode.find('[id*=icon79-]').remove();
      jNode.find('[id*=icon80-]').remove();
  }
}
...
function hide_map(jNode){
  ...
  (function() {var css = "IMG[class=\"leaflet-marker-icon leaflet-zoom-hide leaflet-clickable\"][src*=\"items\/\"]{width:50px !important; height: 25px !important; -moz-border-radius:20px !important; -webkit-border-radius:20px !important; border-radius:20px !important; border: 2px solid magenta !important";
...
function destacarLista(tipo, tag){
  if(tipo == 1){
    $('li:has(a.user.faction:contains("' + tag + '"))').css('background-color', 'lightgreen');
  }
}
if(document.location.href.match(/\/jailview.php/)){
  waitForKeyElements ("ul.user-info-list-wrap.links.icons.users-list.bottom-round", destaca);
}

function destaca(jNode){
  jNode.find('li:has(a.user.faction:contains("' + tagJail + '"))').css('background-color', 'lightgreen');
}
...
if(!document.location.href.match(/\/city.php/)){
  ...
  // Teve de ser movido para aqui pq o Chrome não reconhecia o document.body
    if(document.location.href.match(/\/jailview.php/) || (document.location.href.match(/\/index.php/) && ( document.body.getAttribute('bgcolor') == '#bba47e' || document.body.getAttribute('bgcolor') == '#BBA47E' )) ){
    destacarLista(destacarJail, tagJail); 
    }

    if( document.location.href.match(/\/hospitalview.php/) || (document.location.href.match(/\/index.php/) && document.body.getAttribute('bgcolor') == '#ffffff') ){
    destacarLista(destacarHosp, tagHosp);
    }
...
if (document.location.href.match(/\/companies.php/)) {
  if($("div.details-wrap.player-info").text().match(/Director/) || player == 'alguem'){
...
function employeeLastAction(jNode) {
  ...
  $(".employee-list-wrap").css("background-color","#f2f2f2");
...
function hide_warbase(jNode){
  ...
  if(hideOff == '1'){
    jNode.find("li:has(div.member.icons:has(li[id^='icon2']))").each(function(item){
      $(this).remove();
    });
  }
...
function memberLastAction(jNode) {
  ...
  lista.each( function(){
    var profile = $(this).find('a.user.name').attr('href');
    var ID = profile.match(/\/profiles.php\?XID\=(\d+)/)[1];
  ...
  var linkAPI = "//api.torn.com/faction/?selections=basic&key=";
...
function attack_link(jNode){
  ...
  lista.each( function(){
    var profile = $(this).find('a.user.name').attr('href');
    var ID = profile.match(/\/profiles.php\?XID\=(\d+)/)[1];
    $(this).find('.member').after("<div class='table-cell position' style='width: 8%;'><a class='user name' href='https://www.torn.com/loader.php?sid=attack&user2ID=" + ID + "'>Attack</a></div>");
  });
}
...
\
  ...
  <li><span class="desc"><input type="checkbox" id="obookie"> Bookie Collection</span></li>\
...
<li><span class="desc"><input type="checkbox" id="lactivo6">&nbsp; <input type="text" id="nomeL6" size="10"> \
&nbsp;&nbsp;&nbsp; <input type="text" id="link6" size="35"></span></li>\
<li><span class="desc"><input type="checkbox" id="lactivo7">&nbsp; <input type="text" id="nomeL7" size="10"> \
&nbsp;&nbsp;&nbsp; <input type="text" id="link7" size="35"></span></li>\
  </ul></div></div></div>\
  ...
  $("#obookie").prop('checked', (obookie == '1' ? true : false) ).click(fxchkb);
...
function addLink(no, endereco, texto, nivel, target){
  ...
  novoLink.innerHTML =  '<span>' + '&nbsp; &nbsp; &nbsp; &nbsp; &nbsp;&nbsp;' +  texto+'</span>';
  
  novoDiv.setAttribute('class', row_class);
  novoDiv.style.height = "18px";
  novoDiv.style.paddingTop = "6px";
