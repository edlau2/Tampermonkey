...
async function updateDriversList() {
    ...
    updating = true;
    $('#updating').length < 1 && $('#racingupdatesnew').prepend('<div id="updating" style="color: green; font-size: 12px; line-height: 24px;">Updating drivers\' RS and skins...</div>');
    ...
    updating = false;
    $('#updating').length > 0 && $('#updating').remove();
}
...
function showSpeed() {
    if (!SHOW_SPEED || $('#racingdetails').length < 1 || $('#racingdetails').find('#speed_mph').length > 0)
        return;
...
function showPenalty() {
    if ($('#racingAdditionalContainer').find('div.msg.right-round').length > 0 &&
...
function addSettingsDiv() {
    if ($("#racingupdatesnew").length > 0 && $('#racingEnhSettings').length < 1) {
...
function addExportButton(results, crashes, my_name, race_id, time_ended) {
    if ($("#racingupdatesnew").length > 0 && $('#downloadAsCsv').length < 1) {
...
function addPlaybackButton() {
    if ($("#racingupdatesnew").length > 0 && $('div.race-player-container').length < 1) {
...
//ajax((page, xhr) => {
    ...
    log("Ajax result: ", settings);
    log("xhr: ", xhr);

    if (page != "loader") return log("Page isn't loader");;
    ...
    try {
        log("Going to parse data: ", (xhr ? xhr.responseText : "no xhr"));
        parseRacingData(JSON.parse(xhr.responseText));
    } catch (e) {}

    const JltColor = '#fff200';
    if ($('#racingAdditionalContainer').length > 0 && $('#racingAdditionalContainer').find('div.custom-events-wrap').length > 0) {
        $('#racingAdditionalContainer').find('div.custom-events-wrap').find('ul.events-list > li').each((i, li) => {
            if ($(li).find('li.name').length > 0 && $(li).find('li.name').text().trim().startsWith('JLT-')) {
                $(li).addClass('gold');
