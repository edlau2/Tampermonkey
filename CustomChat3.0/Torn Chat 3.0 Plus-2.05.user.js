...
(function() {
    ...
    */
    ...
    var PRIV_OPTS = {
        ...
        CLOSE_BTN: {val: options.priv.CLOSE_BTN, style: DISPLAY_STYLE,
                      ...
                      fn: 'CLOSE_BTN', display: "Add close btn to minimized chats"},
    };
   ...
   // =============================================================================
    ...
    // Note: can get via GET https://www.torn.com/chat/metadata-v2?rfcv=6838ee8d8b606
    /*
    Response:
    {
        "_id": "6536466a1830230f62d0678a",
        "userId": 2100735,
        "maximized": [
            "faction-8151"
        ],
        "minimized": [
            "private-344411-2100735",
            "private-1886235-2100735",
            "private-2100735-2106526",
            "private-2100735-2607361"
        ],
        "updatedAt": 1748663092052,
        "maximizedMobile": [
            "faction-8151"
        ],
        "minimizedMobile": [
            "private-2100735-2118516"
        ],
        "group": [
            "public_global",
            "public_trade",
    ...
    */
    ...
    function getPrivateMinChats(id) {
        log("[getPrivateMinChats] id: ", id);
        let array = chatMetaData.private;
        log("[getPrivateMinChats] array: ", array);
        const index = array.indexOf(id);
        if (index > -1) {
            array.splice(index, 1);
            log("[getPrivateMinChats] array after slicing: ", array);
        }
        return array;

        /*
        let pChats = [];
        let chats = $(`${PRIVATE_CHAT_SEL}`);
        for (let idx=0; idx<$(chats).length; idx++) {
            //let chat = $(chats)[idx];
            //let chatId = $($(chats)[idx]).attr('id').split(':')[1];

            pChats.push($($(chats)[idx]).attr('id').split(':')[1]);
        }
        return pChats;
        */
    }

    function putChatUpdate(id) {
        ...
        */
        ...
        let myData = {
            "metadata": {
                ...
                "maximized": chatMetaData.minimized,
        ...
        log("Data: ", myData, " stringified: ", payload);

        // Temp - for testing, don't actually send...
        //return;

        /*
        {"metadata":
            {"private":["private-1886235-2100735","private-2100735-3520446","private-1446944-2100735","private-2100735-2874153","private-2100735-3671003"],
            "group":["public_global","public_trade","company","faction"],
            "maximized":["private-344411-2100735","private-1886235-2100735","private-2100735-2106526","private-2100735-2607361"],
            "left":["public_trade","public_global"],
            "updatedAt":1748821908957}}

        {"metadata":
            {"private":["private-1886235-2100735","private-2100735-3520446","private-1446944-2100735","private-2100735-2874153"],
            "group":["public_global","public_trade","company","faction"],
            "maximized":["private-344411-2100735","private-1886235-2100735","private-2100735-2106526","private-2100735-2607361"],
            "left":["public_trade","public_global"],
            "updatedAt":1748822109378}}
        */
        ...
        $.ajax({
            ...
            success: function(response) {
                log('[putChatUpdate] response: ', response);
            },
            error: function(xhr, status, error) {
                console.error('[putChatUpdate] ERROR: ', error);
              }
        });

    }
        /*
        PUT https://www.torn.com/chat/metadata-v2

        {"metadata":{"private":["private-2091413-2100735","private-2100735-2251655","private-2100735-2118516","private-344411-2100735","private-1886235-2100735"],
        "group":["public_global","public_trade","company","faction"],
        "maximized":["faction-8151"],
        "left":["public_trade","public_global"],
        "updatedAt":1748657521425}}:

        ---------

        Close priv chat 2251655 ... AiR

        GET https://www.torn.com/sidebarAjaxAction.php?action=servertime&t=1748659307774&rfcv=6838ee8d8b606
        resp: {time: 1748659307867}

        POST https://www.torn.com/sidebarAjaxAction.php?q=sync&rfcv=6838ee8d8b606
        {fullUrl: "https://www.torn.com/factions.php?step=your&type=1#/tab=armoury"}
         fullUrl: "https://www.torn.com/factions.php?step=your&type=1#/tab=armoury"
        ...
        {"metadata":
            {
            "private":["private-2091413-2100735","private-2100735-2118516","private-344411-2100735","private-1886235-2100735"],
            "group":["public_global","public_trade","company","faction"],
            "maximized":["faction-8151"],
            "left":["public_trade","public_global"],
            "updatedAt":1748659307891
            }
        }:
    ...
    function handleMinClose(e) {
        ...
        log("[handleMinClose]: ", e, $(this), $(root), $(root).attr("id"), id);
        ...
        // Let server know it's gone by updating with what is left - TBD..
        putChatUpdate(id);
    ...
    function delayedHandleCloseBtnOpt(entry, retries=0) {
        ...
        for (let idx=0; idx<$(nodes).length; idx++) {
            ...
            //$(node).parent().addClass('xedx-test');
            //$(node).parent().attr("style", "border: 1px solid limegreen !important;");
    ...
    function handleCbOption(entry) {
        ...
        switch(entry.fn) {
            ...
            // Maybe add observer to parent instead?
            case 'CLOSE_BTN': {
                debug("Close button option!");
                log("subsel: ", entry.subSel, " obj: ", $(`${entry.subSel}`));

                setTimeout(delayedHandleCloseBtnOpt, 500, entry);
                //$(entry.subSel).css("border", "1px solid limegreen !important");
                break;
    ...
    function setPropertyValue(chatType, opt, val) {
        ...
        //log("[setPropertyValue] Variable name: ", variant, " value: ", val);
    }
