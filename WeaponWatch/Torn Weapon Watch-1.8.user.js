...
(function() {
    ...
    // Use this to make unique ID for each entry
    function putWatchOnList(li, entry) {
        let uuid = (entry && entry.uuid) ? entry.uuid : (new Date().getTime());
        if (entry && !entry.uuid) entry['uuid'] = uuid;
        log("Setting uuid to ", entry.uuid);
    ...
    function refreshWatchLists() {
        ...
        function processLookupResult(response, status, xhr, entry) {
            ...
            if (status == 'success' || (response && response.error)) {
                ...
                log("Sel: ", (`li[data-uuid*='${entry.uuid}'] #item-cnt`));
                let node = $(`li[data-uuid*='${entry.uuid}'] #item-cnt`);
                log("node by uuid: ", $(node));
                $(node).text(listings.length);

                log("*** find with uuid: ", (`li[data-uuid*='${entry.uuid}']`), $(`li[data-id*='${entry.uuid}']`));
                log("*** find with uuid: ", entry.uuid, (`li[data-uuid*='${entry.uuid}'] #item-cnt`), $(`li[data-uuid*='${entry.uuid}'] #item-cnt`));
