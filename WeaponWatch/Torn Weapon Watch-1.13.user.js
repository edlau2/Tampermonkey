...
(function() {
    ...
    function handleViewMatches(e) {
        ...
        let searchStr = `&itemID=${itemId}&${htmlName}&itemType=${cat}&sortField=price&sortOrder=ASC&${bonusMod}&from=tww&uuid=${uuid}`;
    ...
    function handleBroadcasts(e) {
        log(tm_str(), " [handleBroadcasts] received: ", e.data.type, " from: ", e.data.myId);

        switch (e.data.type) {
            case "lock-query": {
                log("[handleBroadcasts] lock-query, my ID: ", myId, " their ID: ", e.data.myId);
            ...
            case "lock-response": {
                log("[handleBroadcasts] lock-response, my ID: ", myId, " their ID: ", e.data.myId);
                log("[handleBroadcasts] my lock: ", notifyLock);
                log("[handleBroadcasts] their lock: ", e.data);
    ...
    function handleNotifyClick(context) {
        ...
        log("Notification clicked, visibility: ", document.visibilityState, "\ncontext: ", context, "\nurl: ", context.url);
    ...
    function getSearchUrlByEntry(entry) {
        ...
        let searchStr = `&itemID=${entry.itemId}&${htmlName}&itemType=${entry.cat}&sortField=price&sortOrder=ASC&${bonusMod}&from=tww&uuid=${entry.uuid}`;
        log("Search: ", searchStr);
    ...
    function doBrowserAlert(msg, entry) {
        ...
        log(tm_str(), " [handleBroadcasts] post broadcast: ", notifyLock);
        channel.postMessage(notifyLock);
        ...
        function realBrowserAlert(msg, entry) {
            log(tm_str(), " [realBrowserAlert] Timer Popped! ");
            ...
            log(tm_str(), " [realBrowserAlert] timeout: ", (timeout/1000), " seconds");
            log(tm_str(), " [realBrowserAlert] resetLock: ", (lockDelay/1000), " seconds");
            ...
            let opts = {
                ...
                url: url,
                onclick: (context) => {
                    log(tm_str(), " [realBrowserAlert] notification clicked: ", entry, notifyLock);
                    handleNotifyClick(context);
                }, ondone: () => {
                    log(tm_str(), " [realBrowserAlert] notification done: ", entry, notifyLock);
        ...
        function resetLock() {
            log(tm_str(), " [resetLock] Unlocking! ", notifyLock);
            notifyLock.locked = false;
            notifyLock.remoteLocked = false;
            notifyLock.lockedFor = 0;
            notifyLock.owner = '';
            notifyLock.unlockTime = 0;
            log(tm_str(), " [resetLock] Unlocked: ", notifyLock);
        }
    }
