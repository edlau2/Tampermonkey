...
(function() {
    ...
    */
    ...
    const commonEntries = ["No Bonus Filter", "Any", "Double", "Yellow", "Orange", "Red"];
    var bonusByType = {
    ...
    function findObjectById(array, key, value) {
        return array.find(obj => obj && obj.hasOwnProperty('id') && obj.id === parseInt(value));
    }
    ...
    function fillBonusLists(li) {
        ...
        let entry = findObjectById(optList, "id", parseInt(itemId));
    ...
    function buildWatchList() {
        ...
        for (let idx=0; idx<$(watches).length; idx++) {
            ...
            let entry = {id: $(li).attr('id'), enabled: enabled, cat: $(li).find('.weap-cat').val(),
                         name: $(li).find('.weap-name').val(), bonus1: $(li).find('.weap-bonus1').val()};
    ...
    function saveWatches() {
        ...
        for (let idx=0; idx<$(watches).length; idx++) {
            let li = $(watches)[idx];
            let itemId = $(li).attr('id').replace("ww-", "");
            let entry = {id: $(li).attr('id'), cat: $(li).find('.weap-cat').val(),
                         name: $(li).find('.weap-name').val(), bonus1: $(li).find('.weap-bonus1').val(),
                         bonus2: $(li).find('.weap-bonus2').val()};
            savedWatches.push(entry);
        }
        */
    ...
    function refreshWatchLists() {
        ...
        function doEntryLookup(entry) {
            ...
            if (entry.bonus1 == 'No Bonus Filter')
