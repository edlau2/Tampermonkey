...
(function() {
    ...
    */
    ...
    function reenablePageButtons() {
        let pageList = $(".pagination-wrap .pagination > .page-number");
        $(pageList).css("opacity", 1);
        $(pageList).off('click.xedx');
    }
    ...
    function restorePage() {
        ...
        let orgItems = findObjectsByKey(armoryWeapons, 'page', (thisPage - 1));
        log("[restorePage] ", thisPage, orgItems);

        let ul = $(`#armoury-weapons > ul.item-list`);
        $(ul).empty();

        for (let idx=0; idx<50 && idx<orgItems.length; idx++) {
            let entry = orgItems[idx];
    ...
    function replacePageEntriesFromArray(pageNum, srcArray) {
        ...
        // Sort array first
        // { armoryID: item.armoryID, itemID: id, name: name, page: currPage, index: idx, type: item.type,  loaned: item.loaned,
        // glowClass: item.glowClass, acc: item.acc, dmg: item.dmg, bonus1: bonus1, bonus2: bonus2}
        let sortKey = $('#sort-by').find(":selected").val();
        itemListSortByKeyVal(srcArray, sortKey);

        let idxOffset = (thisPage - 1) * 50;
        log("idxOffset: ", idxOffset);

        for (let idx=0; idx<50 && (idx + idxOffset)<srcArray.length; idx++) {
            let entry = srcArray[idx + idxOffset];
            let newLi = getItemLi(entry.armoryID);
            $(ul).append(newLi);
        }
    ...
    function handleBonusSelect() {
        reenablePageButtons();
        let bonus = $(this).val();  //  $('#search-for').find(":selected").val();
        if (bonus == 'none') {
            log("none selected - restore");
            return;
        }
    ...
    function doBonusSearch() {
        let bonus = $('#search-for').find(":selected").val();
        log("[doBonusSearch] bonus: ", bonus);
        if (bonus == 'none')
            restorePage();
        else
            ...
            <div id="xarmory">
                <span class="xflexr">
                    ...
                    <!-- div class="armory-box" -->
                        ...
                        <input id="xarmory-test" class="xedx-torn-btn" value="Search">
                    <!-- /div -->
                </span>
            </div>
        `;
            ...
            `<li>
             ...
             + `</div>
                <div class="item-action">` +
                  ...
                  +  ((item.itemActions && item.itemActions.give) ?
