...
(function() {
    ...
    function handleSortClick(e) { scrollToMyLi(myFacTarget);}
    ...
    function scrollToMyLi(root) {
        if (!root) root = myFacTarget;
        log("[scrollToMyLi] ", $(root).length, $(root));

        //$(`ul.members-names-rows > li > div > div[class^='honorWrap_'] > a[href="/profiles.php?XID=${userId}"]`);
    ...
    function addTargetHandlers() {
        ...
        if (scrollToMyLi(myFacTarget)) scrollTo(myLi);
