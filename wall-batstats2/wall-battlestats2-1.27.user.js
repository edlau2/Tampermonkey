  ...
  "use strict";
    ...
    function initColWidths(retries=0) {
        ...
        if (enableScrollLock == true) {
            $("ul.table-header").css({"position": "sticky", "top": 0, "z-index": 99999});
    ...
    // ========================= Options panel installation =========================

    const statUpdTT = "How often to update member status (online, idle, offline, etc). Applies during war only. Value is in seconds.";
    const updFreqTT = 'placeholder';

    const optEntries = [
        {id: 'bs-show-hosp', type: 'cb', enabled: true, title: 'Display hosp time left'},
        {id: 'bs-api-key', type: 'input-text', title: 'Your API key', value: ''},
        {id: 'bs-stat-upd-freq', type: 'input-num', title: 'Status update frequency, war', value: 5, tooltip: `${statUpdTT}`},
        {id: 'bs-stat-upd-freq', type: 'input-num', title: 'Status update frequency', value: 5, tooltip: `${updFreqTT}`},
        {id: 'bs-scroll-lock', type: 'cb', enabled: true, title: 'Scroll member list independently'},
        {id: 'bs-arr-times', type: 'cb', enabled: true, title: 'Experimental: estimate flight arrival times'},
    ];
    function doOptionsPanel(e) {
        debug("[doOptionsPanel]");

        $(`#xonline-title-${enemyID}`).after(getOptsDiv());
        optEntries.forEach(entry => { addOptLi(entry); });

        function addOptLi(entry) {

        }
        function getOptsDiv() {
            let optsDiv = `
                <div id='xbs-opts-wrap'>
                   <ul class='table-body'>
    ...
    // ===== Bar along the top that shows active/idle/offline/OK etc user stats =====
    function installUsersBar(facId, retries=0) {
        ...
        $("#xedx-options-btn").on('click', doOptionsPanel);
            ...
            <div id="xonline-title-${ID}" class="title-black m-top10">
                <div class="counts-wrap">
                    ...
                    <div class="counts-wrap xml20">
                        ...
                        <input id="xedx-options-btn" type="submit" class="xedx-torn-btn xmr10 xmt3" value="Options">
