...
// @require     https://raw.githubusercontent.com/edlau2/Tampermonkey/master/helpers/HoverTip.js
...
// TBD: Finish subsort ...
// file:////Users/edlau/Documents/Documents - Ed’s MacBook Pro/Tampermonkey Scripts/Helpers/HoverTip.js
...
(function () {
  "use strict";
    ...
    // tooltip class. Code is in HoverTip.js
    const tips = new HoverTip({
      selector: ".members-list > ul.table-body > li",
      mode: 'anchor',
      placement: 'top',
      offset: 8,
      follow: false,      // reposition on scroll/resize if true
      persist: false,
      className: 'hovertip'
    }).install();

    let loadFactionsDoneOnce = false;
    var enemyMembers = {};
    ...
    var gettingMyFac = false, gettingEnemyFac = false;
    ...
    let bs2ShowStatsTimer = null;
    let bs2LastShowStats = 0;

    function scheduleShowStatsAll(node = null, delay = BS2_LITE.refreshUiMs) {
        if (bs2ShowStatsTimer) return;

        bs2ShowStatsTimer = setTimeout(() => {
            bs2ShowStatsTimer = null;

            const now = Date.now();
            if (now - bs2LastShowStats < BS2_LITE.minShowStatsGapMs) return;
            bs2LastShowStats = now;

            try {
                showStatsAll(node);
            } catch (e) {
                console.error("[wall-battlestats2-lite] showStatsAll failed", e);
            }
        }, delay);
    }

    let bs2WarListClickTimer = null;
    function scheduleAddWarListClickHandlers(delay = BS2_LITE.warListClickMs) {
        clearTimeout(bs2WarListClickTimer);
        bs2WarListClickTimer = setTimeout(() => {
            bs2WarListClickTimer = null;
            try {
                addWarListClickHandlers();
            } catch (e) {
                console.error("[wall-battlestats2-lite] addWarListClickHandlers failed", e);
            }
        }, delay);
    }

    const sInMin = 60;
    const sInHr = sInMin * 60;
    const sInDay = sInHr * 24;
    ...
    function validFacPage() {
        const facTabList = ['territory', 'rank', 'crimes', 'upgrades', 'armoury', 'controls'];
        checkPageParams();
    ...
    function hashChangeHandler() {
        ...
        //
        // I don't remember why I had to do this, whatever it was for,
        // prob be easier to just check the page before doing it,
        // the reload adds a wierd effect
        ...
        if (isValidFacPage == false) {
            if (refreshTimer) clearInterval(refreshTimer);
            refreshTimer = null;
        } else {
            setTimeout(startRefreshTimer, 100);
        }

        scheduleAddWarListClickHandlers(250);
    }
    ...
    // Too many reqs per minute - slow things down
    var apiPaused = false;
    var pauseTimeout = null;
    const resumeApiCalls = () => { debug("[resumeApiCalls]"); apiPaused = false; pauseTimeout = null;}

    function pauseApiCalls() {
        debug("[pauseApiCalls] paused? ", apiPaused);

        apiPaused = true;
        if (pauseTimeout) clearTimeout(pauseTimeout);
        pauseTimeout = setTimeout(resumeApiCalls, 10000);
    }
    ...
    function userProfileQueryCb(responseText, ID, param) {
        ...
        log("***** [userProfileQueryCb] json: ", jsonObj, " profile: ", jsonObj.profile);
        if (jsonObj.profile) {
            myFacId = jsonObj.profile.faction_id;
            GM_setValue("myFacId", myFacId);
            log("[userProfileQueryCb] facId: ", myFacId);
    ...
    function addPositionTtToList(nodeList) {
        checkPageParams();
        if (!nodeList && ourProfile == true) {
            nodeList = $(".members-list > ul.table-body > li");
        }

        if ($(nodeList).length < 0) return log("[addPositionTtToList] ERROR empty list! ourProfile: ", ourProfile);

        log("[addPositionTtToList] nodeList: ", $(nodeList));

        tips.setForEach($(nodeList), ($li, i) => `# ${i + 1}`);

        // for (let idx=0; idx<$(nodeList).length; idx++) {
        //     let elem = $(nodeList)[idx];
        //     displayHtmlToolTip($(elem), ("#" + ($(elem).index()+1)), "tooltip5");
        // }
    }
    ...
    function installExtraUiElements() {
        ...
        // Add handler to show our member's position in the table
        // need to do on sorting, call on tab click?
        log("[installExtraUiElements] ourProfile: ", ourProfile);
        if (ourProfile == true) {
            let memberList = $(".members-list > ul.table-body > li");
            log("[installExtraUiElements] calling addPositionTtToList memberList: ", $(memberList));
            addPositionTtToList(memberList);
    ...
    function loadFactions() {
      let factionIds = Array.from(
          //document.querySelectorAll(".enemy [href^='/factions.php?step=profile&ID=']")
          //document.querySelectorAll("[href^='/factions.php?step=profile&ID=']")
          document.querySelectorAll("[class*='rankBox_'] [class*='FactionName_'][href^='/factions.php?step=profile&ID=']")
      )
        .map((a) => a.href.replace(/.*?ID=(\d+)$/, "$1"))
        .filter((v, i, a) => a.indexOf(v) === i);

        debug("factionIDs: ", factionIds);
        factionIds.forEach((id) => loadTSFactions(id));
    }

    // ====================================================================================

    function timeStrToSec(tmStr) {
        //debug("**** [timeStrToSec] tmStr: ", tmStr);
        let tm = 0;
        if (tmStr) {
            let parts = tmStr.split(":");
            let len = parts ? parts.length : 0;
            //debug("**** [timeStrToSec] parts: ", parts, " len: ", len);
            if (!len) return 0;
            if (len == 4) {
                tm = tm + (parseInt(parts[0]) * sInDay) +
                          (parseInt(parts[1]) * sInHr) +
                          (parseInt(parts[2]) * sInMin) +
                          parseInt(parts[3]);
            } else if (len == 3) {
                tm = tm + (parseInt(parts[0]) * sInHr) +
                          (parseInt(parts[1]) * sInMin) +
                          parseInt(parts[2]);
            } else if (len == 2) {
                tm = tm + (parseInt(parts[0]) * sInMin) +
                          parseInt(parts[1]);
            }
        }

        //debug("**** [timeStrToSec] tm: ", tm);
        return tm;
    }

    var subSortLoTm = 9999999999;
    var subSortTmr = null;
    var noSubSort = true;
    var ffEstDone = false;
    function hospTimeForSort(li) {
        let tm = 0;
        let statNode = $(li).find(".hospital.not-ok");
        let tmStr = $(li).find(".hospital.not-ok").text();
        if (tmStr) tm = timeStrToSec(tmStr);
        //debug("**** [hospTimeForSort] tm, tmStr: ", tm, tmStr);
        if (tm <= 0) {
            debug("*** hospTimeForSort tm is 0!!!");
            $(li).removeClass('bs-xhosp');
            $(li).attr("data-tmp-tm", '');
            $(statNode).removeClass("not-ok");
            //$(statNode).text('--');
            return 0;
        }
        if (tm > 0 && tm < subSortLoTm) subSortLoTm = tm;
        $(li).attr("data-tmp-tm", tm);
        //debug("**** [hospTimeForSort] new li: ", $(li));
    }

    function subSortHospTimes() {
        debug("**** [subSortHospTimes] noSubSort: ", noSubSort);
        if (subSortTmr) {
            clearTimeout(subSortTmr);
            subSortTmr = null;
        }
        if (noSubSort == true) return debug("*** noSubSort, bail");;

        let hospList = $(".bs-xhosp");
        //debug("**** [subSortHospTimes] ", $(hospList));
        $(hospList).attr("data-temp-tm", '');
        for (let idx=0; idx<$(hospList).length; idx++) {
            let li = $(hospList)[idx];
            //debug("**** [subSortHospTimes] li: ", $(li));
            // This adds "data-tmp-tm" to the li to sort by
            hospTimeForSort(li);
        }
        // do the sort...
        tinysort($(hospList), {attr: "data-tmp-tm", order: "asc" });
        debug("****** [subSortHospTimes] setting timeout: ", subSortLoTm);
        subSortTmr = setTimeout(subSortHospTimes, (5 * 1000));

        if (ffEstDone == false && $(".ff-scouter-est-visible").length > 0) {
            $(".ff-scouter-est-visible").on('click', function() {
                clearTimeout(subSortTmr);
                subSortTmr = null;
                noSubSort = true;
            });
    ...
    function expSort(from="def", doSubSort=false, altList=null) {
        debug("*** [expSort] from: ", from);
        let sortClass = GM_getValue("sortClass");
        // #react-root > div > div.faction-info-wrap.restyle.another-faction > div > ul.table-body
        let sortList = altList ? $(altList) : $(".faction-info-wrap  ul.table-body > li");
        ...
        if (from == 'tableReady') {
            ...
            // log("[expSort] calling addPositionTtToList");
            // addPositionTtToList();
            return;
        }
        ...
        // If desired, can do a secondary sort, for example hosp time after status...
        // debug("**** [sortHospTimes} class: ", sortClass, from, doSubSort);
        if ((sortClass == 'status' && from == "hdrClick")|| doSubSort == true) {
            debug("**** [sortHospTimes} do subsort!! noSubSort: ", noSubSort, sortClass, from, doSubSort);
            noSubSort = false;
            subSortHospTimes();
        } else {
            debug("**** [sortHospTimes} turn off subsort!! noSubSort: ", noSubSort, sortClass, from, doSubSort);
            clearTimeout(subSortTmr);
            subSortTmr = null;
            noSubSort = true;
        }
        ...
        // log("[expSort] calling addPositionTtToList (2)");
        // addPositionTtToList();
    }
    ...
    var memStatusRetry = null;
    function memberStatusRefresh() {
        logApiCall("fac members: " + enemyID);

        if (apiPaused == true) return;

        // if (apiPaused == true) {
        //     if (memStatusRetry) clearTimeout(memStatusRetry);
        //     memStatusRetry = setTimeout(memberStatusRefresh, 2000, retries);
        //     return;
        // }
        // memStatusRetry = 0;
        ...
        function memberRefreshCb(responseText, ID, param) {
            ...
            debug("Entering time-crit region");
            let needSort = false;
            ...
            membersArray.forEach(member => {
                ...
                let prevState = useArray[member.id] ? useArray[member.id].prevState : '';

                if (useArray[member.id]) {
            ...
            debug("Exiting time-crit region");
    ...
    function getEnemyId() {
        ...
        let link = $("[class*='rankBox_'] [class*='opponentFactionName_'][href^='/factions.php?step=profile&ID=']");
    ...
    function getMyFacId() {
        ...
        xedx_TornUserQueryv2('', "profile", userProfileQueryCb);

        // let link = $("[class*='rankBox_'] [class*='currentFactionName_'][href^='/factions.php?step=profile&ID=']");
        // let href =$(link).attr("href");
        // let id = href ? href.split("ID=")[1] : null;
        // return id;
    }
    ...
    function startRefreshCb(responseText, ID, param) {
        ...
        if (apiPaused == true) return debug("memberStatsuRefresh API paused!");

        if (refreshTimer) {
            clearInterval(refreshTimer);
            refreshTimer = null;
        }
        // diagnostic: disable recurring faction-member refresh loop
        if (isValidFacPage == true)
            refreshTimer = setInterval(memberStatusRefresh, refreshTimerDelay);
    }

    var enemyRetry = null;
    function getEnemyMembers(retries=0) {
        if (apiPaused == true) return;

        // if (apiPaused == true) {
        //     if (enemyRetry) clearTimeout(enemyRetry);
        //     enemyRetry = setTimeout(getEnemyMembers, 2000, retries);
        //     return;
        // }
        // enemyRetry = 0;
        ...
        if (!enemyID) {
            enemyID = getEnemyId();
            if (!enemyID) {
                if (retries++ < 25) return setTimeout(getEnemyMembers, 250, retries);
                return debug("No enemy ID, timed out");
            }
        }

        logApiCall("enemy members: " + enemyID);
    ...
    function getFacMembers(retries=0) {
        ...
        // if (apiPaused == true) {
        //     if (facRetry) clearTimeout(facRetry);
        //     facRetry = setTimeout(getFacMembers, 2000, retries);
        //     return;
        // }
        // facRetry = 0;
        ...
        if (!myFacId) {
            getMyFacId();
            if (retries++ < 25) return setTimeout(getFacMembers, 250, retries);
            return debug("No my fac ID, timed out");
        }
            // if (!myFacId) {
            //     if (retries++ < 25) return setTimeout(getFacMembers, 250, retries);
            //     return debug("No my fac ID, timed out");
        ...
        logApiCall("get fac members: " + myFacId);
    ...
    // var refreshTimerDelay = 5000;    // 12 per minute
    // var gettingMyFac = false, gettingEnemyFac = false;
    var activeRefreshTimer = 0;
    var lastTimeEnter = 0;
    function startRefreshTimerWrap(retries) {
        activeRefreshTimer = 0;
        startRefreshTimer(retries);
    }

    function startRefreshTimer(retries=0) {
        let now = new Date().getTime();
        if (now - lastTimeEnter < 2000) {
            debug("[startRefreshTimer] time dif < 2 sec! ", now, lastTimeEnter, (now - lastTimeEnter));
            return;
        }
        lastTimeEnter = now;
        debug("[startRefreshTimer] enemyID: ", enemyID, " mine: ", myFacId);

        if (!myFacId) {
            getMyFacId();
            if (!activeRefreshTimer && retries++ < 20) {
                activeRefreshTimer = setTimeout(startRefreshTimerWrap, 250, retries);
                return;
            }
            return debug("No my fac ID, timed out");
        }
        ...
        if (Object.keys(myFacMembers).length < 5 && gettingMyFac == false) {
            getFacMembers();
            gettingMyFac = true;
        }
        if (Object.keys(myFacMembers).length > 5) gettingMyFac = false;
        ...
        if (!Object.keys(enemyMembers).length && gettingEnemyFac == false) {
            getEnemyMembers();
            gettingEnemyFac = true;
        }
        if (Object.keys(enemyMembers).length > 0) gettingEnemyFac = false;
    ...
    function updateHospTimers() {
        if (!enemyMembers || !Object.keys(enemyMembers).length) {
            if (updateAttempts++ > 25) return;
            if (gettingEnemyFac == false) {
                getEnemyMembers();
                gettingEnemyFac = true;
                return;
            }
            //setTimeout(updateHospTimers, 2000);
            //return debug("No enemy lists yet: ", gettingEnemyFac);
        }
        if (!myFacMembers || Object.keys(myFacMembers).length < 5) {
            setTimeout(updateHospTimers, 2000);
            return debug("No member lists length yet: ", gettingMyFac);
        }
        ...
        setTimeout(updateHospTimers, BS2_LITE.hospMs);
    }
    ...
    function watchWall(observeNode) {
      ...
      if (!loadFactionsDoneOnce) {
          loadFactionsDoneOnce = true;
          //loadFactions();
      }
      ...
      let prevSortCheck = "";
      const wallObserverCb = throttleTrailing((mutations) => {
        ...
        const mo = new MutationObserver(wallObserverCb);
    ...
    function watchWalls(observeNode) {
       ...
       new MutationObserver(throttleTrailing((mutations) => {
            mutations.forEach((mutation) => {
                for (const node of mutation.addedNodes) {
                    node.querySelector &&
                    ...
                    if ($(node).hasClass("descriptions")) {
                        scheduleAddWarListClickHandlers();
                    }
                }
            });
        }, BS2_LITE.wallsObsMs)).observe(observeNode, { childList: true, subtree: true });
    }
    ...
    // diagnostic: disable recurring timers
    setTimeout(updateHospTimers, BS2_LITE.hospMs);
    startRefreshTimer();
    ...
    //         }
    //     });
    // }, BS2_LITE.bodyObsMs)).observe(document.body, { childList: true, subtree: true });
    ...
    unsafeWindow.WebSocket = function (...args) {
      ...
      socket.addEventListener("message", throttleTrailing((event) => {
    ...
    const tornFlag = `<li style=margin-bottom: 0px;"><img class="flag selected bs2 bs-flag" src="/images/v2/travel_agency/flags/fl_torn.svg"
    ...
    let iconObserversInstalled = false;
    function installMemberIconObservers() {
        if (iconObserversInstalled == true) return;
        iconObserversInstalled = true;

        // Need 2 observers? Or one...hmm...
    ...
    var replaceTravelIconsBusy = false;
    function replaceTravelIcons(retries=0) {
        if (replaceTravelIconsBusy) return;
        replaceTravelIconsBusy = true;

        try {
            if (ourProfile == true || getPageTab() == 'info') return;
            if (apiPaused == true) return debug("[replaceTravelIcons] apiPaused");

            installMemberIconObservers();
            if (!travelTimer)
                travelTimer = setInterval(replaceTravelIcons, BS2_LITE.travelMs);
            ...
            for (let i=0; i < $(combinedArray).length; i++) {
                ...
                if (replaceDestText(id, div) == "error") break;
            }

            function replaceDestText(userId, textNode) {
                if (apiPaused) return debug("[replaceDestText] apiPaused");

                if (!enemyMembers || !Object.keys(enemyMembers).length) {
                    debug("No enemy member list yet");
            ...
            function replaceDestFlag(userId, iconLi) {
                ...
                function getAbroadFlag(country) {
                    ...
                    return `<li class="bs2" style="margin-bottom: 0px; z-index: 9999;"><img class="flag selected bs-flag" src="/images/v2/travel_agency/flags/fl_${entry.flag}.svg"
                            country="${entry.ctry}" alt="${entry.title}" title="${entry.title}"></li>`;
                }
            }

        } finally {
            replaceTravelIconsBusy = false;
        }
    ...
    */
    ...
    // TBD: not complete, seems attck column gets hidden
    function addAttackColumn() {
        log("[addAttackColumn]");
        let attHdrs = $(".enemy-faction [class*='tab'].attack");
        let statusHdrs = $(".enemy-faction [class*='tab'].status");

        // if (hdrsLogged == false && $(list).length > 0) {
        //     log("attHdrs: ", $(attHdrs), " statusHdrs: ", $(statusHdrs));
        //     hdrsLogged = true;
        // }

        if ($(attHdrs).length && !$($(attHdrs)[0]).hasClass("xattack")) {
            $($(attHdrs)[0]).addClass("xattack");
            let divList = $("li.enemy > div.attack");
            for (let idx=0; idx < $(divList).length; idx++) {
                let div = $(divList)[idx];
                $(div).addClass('xattack');
            }
        }
        if ($(statusHdrs).length && !$($(statusHdrs)[0]).hasClass("xstatus")) {
            $($(statusHdrs)[0]).addClass("xstatus");
            let divList = $("li.enemy > div.status");
            for (let idx=0; idx < $(divList).length; idx++) {
                let div = $(divList)[idx];
                $(div).addClass('xstatus');
            }
        }
    }
    ...
    function addWarListClickHandlers() {
        ...
        // If have list but no att hdrs, add
        if ($(list).length > 0 && !$("li.enemy > div.xattack").length) {
            addAttackColumn();
        }
    ...
    function initColWidths(retries=0) {
        ...
        GM_addStyle(`
            .xstatus {
                ...
                border-right: 1px solid var(--default-panel-divider-outer-side-color) !important;
            }
            .xattack {
    ...
    function handleTableHeaderClick(e) {
        ...
        if ($(bsTableHeader).outerWidth() > 40) {
            debug("[handleTableHeaderClick] bad hdr width: ", $(bsTableHeader).outerWidth(), $(bsTableHeader));
            //debugger;
        }
        ...
        // Do the sort? Why aren't we???
        expSort("hdrClick", true);
