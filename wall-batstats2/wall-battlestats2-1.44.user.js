...
(function () {
  "use strict";
    ...
    function startRefreshTimer(retries=0) {
        ...
        if (!myFacId) {
            getMyFacId();
            if (retries++ < 25) return setTimeout(startRefreshTimer, 250, retries);
            return debug("No my fac ID, timed out");
        }
        retries = 0;
...
//         log("[validFacPage] queryString: ", queryString);
//         const urlParams = new URLSearchParams(queryString);

//         let step = urlParams.get("step");
//         let type = urlParams.get("type");
//         log("[validFacPage] step: ", step, " type: ", type);

//         if (step == 'profile') return true;
//     }

    // Don't do this on any pages but main fac tab! (on our face page)
    log("[validFacPage] facTab: ", facTab);
    // if (validFacPage() == true) {
        log("[validFacPage] valid fac page");
        setTimeout(updateHospTimers, 1000);
        startRefreshTimer();
    // } else {
    //     log("[validFacPage] invalid page");
    // }
