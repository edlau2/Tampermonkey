...
(function() {
    ...
    var fastSearchEnabled = GM_getValue("fastSearchEnabled", true);
    const onUserListPage = location.href.indexOf("UserList") > -1;
    ...
    function doAjaxPost(startNum, init=false) {
        if (!fastSearchEnabled || !onUserListPage) return;
    ...
    function handleFusCb(e) {
        GM_setValue('fastSearchEnabled', $("#xfus-cb").prop('checked'));
        fastSearchEnabled = GM_getValue('fastSearchEnabled', fastSearchEnabled);
    }

    function doAdvSearchEdit(retries=0) {
        log("doAdvSearchEdit, retries: ", retries, " len: ", $("#xfus-span").length);
        if ($("#xfus-span").length > 0) return;

        let optCb = `
            <span id="xfus-span">
                <input type="checkbox" id="xfus-cb">
                Fast User Search
            </span>`;

        let advSrchLi = $(".find-extend");
        let advSrchBody = $(".advancedSearchFormBody");
        let advSrchBtm = $(".advancedSearchFormBody").parent().find(".bottom");

        if (!$(advSrchBody).length) {
            if (retries++ < 10) return setTimeout(doAdvSearchEdit, 200, retries);
            return log("Too many retries");
        }

        $(advSrchBtm).prepend(optCb);
        $("#xfus-cb").prop('checked', fastSearchEnabled);
        $("#xfus-cb").on('click.xedx', handleFusCb);
    }

    function handlePageLoad() {
        log("Adding search opts");
        doAdvSearchEdit();
        if (!fastSearchEnabled || !onUserListPage) {
            log("Fast search not enabled! going home.");
            return;
        }
    ...
    addSearchStyles();
    callOnContentLoaded(doAdvSearchEdit);
