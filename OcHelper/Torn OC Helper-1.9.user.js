...
(function() {
    ...
    const F = function(x) { return Math.floor(x); }
    function formatSecsDiff(diffSecs) {
        let days = F(diffSecs / secsInDay);
        let remains = (diffSecs - (days * secsInDay));
        let hrs = F(remains / secsInHr);
        remains = remains - (hrs * secsInHr);
        let mins = F(remains/60);

        return ("OC in: " + Math.abs(+days) + "d " + nn(hrs) + "h " + nn(mins) + "m");
    }
    ...
    class OcTracker {
        ...
        setTrackerTime(time) {
            ...
            if (myOcTracker.timeMin(time) < 1) myOcTracker.stopAllAlerts();

            if (myOcTracker.missingReqItem == true) {
                $("#missing-item").addClass("blinkOcWarnBtn");
                $("#missing-item").css("display", "flex");
            } else if (myOcTracker.missingReqItem == false) {
                $("#missing-item").removeClass("blinkOcWarnBtn");
                $("#missing-item").css("display", "none");
            }

            if (notifyOcSoon == true && +notifyOcSoonMin > 0) {
                let untilMin = myOcTracker.timeMin(time);
                if (untilMin && untilMin < +notifyOcSoonMin)
                    myOcTracker.enableOcSoon();
            }
        }

        secTimer = null;
        updateDisplayTime(diffSecs) {
            diffSecs = parseInt(diffSecs) - 1;

            let now = new Date();
            let dt = myCrimeStartDate;
            let savedTime = parseFloat(GM_getValue("myCrimeStartTime", 0));
            if (savedTime > 0) {
                let savedDt = new Date(savedTime * 1000);
                let orgDiff = diffSecs;
                diffSecs = parseInt(savedTime) - parseInt(now.getTime()/1000);
            }

            myOcTracker.setTrackerTime(formatSecsDiff(diffSecs));
            let r = Math.abs(diffSecs % 60);
            let interval = (r <= 2) ? 500 : (r <= 10) ? 2100 : 5250;
            if (document.visibilityState === 'visible') {
                myOcTracker.secTimer = setTimeout(myOcTracker.updateDisplayTime, interval, diffSecs);
            }
        }

        getDisplayTimeUntilOc() {
            if (!myCrimeStartTime) {
                myOcTracker.scheduleOcCheck(5000);
                return NO_OC_TIME;
            }
            ...
            if (myOcTracker.secTimer) clearTimeout(myOcTracker.secTimer);
            let int = (diffSecs % 60 > 5) ? 5000 : 1000;
            myOcTracker.secTimer = setTimeout(myOcTracker.updateDisplayTime, int, diffSecs);
            return formatSecsDiff(diffSecs);
        }
        ...
        toggleNoOcWarning() {
            if (blinkingPaused) return myOcTracker.stopAllAlerts();
            $("#oc-tracker-time").toggleClass("blinkOcWarn");
        ...
        handleOcResponse(responseText, ID, param) {
            ...
            if (myCrime && readyAt) {
                ...
                myCrimeStartTime = (status == "Recruiting") ? planningAt : readyAt;
                debug("*** Pick OC time: ", status, ((status == "Recruiting") ? "planningAt" : "readyAt"));
                let dt = new Date(myCrimeStartTime * 1000);

                // Save to re-synce paused tabs
            ...
            myOcTracker.missingReqItem = false;
                ...
                if (slot.user_id == userId) {
                    if (slot.item_requirement != null) {
                        if (slot.item_requirement.is_available == false) {
                            myOcTracker.missingReqItem = true;
                            myOcTracker.missingReqItemId = slot.item_requirement.id;
                            let key = "missingItemId-" + myOcTracker.missingReqItemId;
                            myOcTracker.missingReqItemName = GM_getValue(key, null);
                            if (!myOcTracker.missingReqItemName) {
                                xedx_TornTornQuery(myOcTracker.missingReqItemId, "items",
            ...
            if (myCrimeStartTime) {
                ...
                setInterval(myOcTracker.updateTrackerTime, 31000, myOcTracker);
            }
        ...
        getMyNextOcTime(force) {
            ...
            if (!force && diff < 20000) {
                debug("Last call was less than 20 secs ago!", diff);
                return;
            }
            myOcTracker.lastQueryTime = now;
            if (!force && document.visibilityState != 'visible') {
                debug("Document not visible, don't call the API");
                return;
            }
            xedx_TornUserQueryv2("", "organizedcrime", myOcTracker.myOcTrackingCb);
        }
        ...
        startMyOcTracking() {
            ...
            $("#oc2Timer").remove();
            let elem = myOcTracker.getMyOcTrackerDiv();
            ...
            myOcTracker.installOcTrackerContextMenu();
            myOcTracker.scheduleOcCheck(250, 'available');
        ...
        parseOcTime(time) {
            ...
            return {d: parseInt(parts[2]), h: parseInt(parts[3]), m: parseInt(parts[4])};
        }
        ...
        installOcTrackerContextMenu() {
            ...
            for (let idx=0; idx<myOcTracker.contextMenuItems.length; idx++) {
                ...
                let optionalClass =
                    (idx == 0) ? "opt-first-li" :
                    (idx == myOcTracker.contextMenuItems.length - 1) ? "opt-last-li" : "";
