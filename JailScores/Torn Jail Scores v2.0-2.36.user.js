...
(async function() {
    ...
    */
    ...
    function startMidnightTimer(timerCallback, param1, param2) {
        ...
        const delay = midnightUTC.getTime() - now.getTime();
    ...
    function addJailScores(retries=0) {
        log('[addJailScores] retries: ', retries);
        ...
        if (elemList.length == 0) {
            log("[addJailScores] No user list! Retries: ", retries);
            return setTimeout(addJailScores, 500, retries++);
        }
        var items = elemList[0].getElementsByTagName("li");
        if (items.length <= 1) {
            log("[addJailScores] No list elements! Retries: ", retries);
            return setTimeout(addJailScores, 500, retries++);
        }
        ...
        for (var i = 0; i < items.length; ++i) {
            ...
            log("[addJailScores] scoreNode: ", $(scoreNode).length);
            if ($(scoreNode).length) {
        ...
        log("[addJailScores] scores calculated");
    ...
    function setTodaysBusts(numBusts, retries=0) {
        log("*** dailyBusts: setTodaysBusts: ",
              numBusts, " alt local: ", GM_getValue("currBustsAlt", -1),
            " alt utc: ", GM_getValue("currBustsUtcAlt", -1),
           "\nspan: ", $("#busts-today .numBusts"));
    ...
    function processResponse(response, pageNum) {
        ...
        if (jsonObj.success == true) {
            ...
            log("Reload complete, sorting and scoring: ", scoresCalculated);
