...
(async function() {
    ...
    // Incomplete!
    function writeOpts() {
        GM_setValue("extraExtraDebug", extraExtraDebug);
        GM_setValue("debugLoggingEnabled", debugLoggingEnabled);
        GM_setValue("debugPastBusts", debugPastBusts);
    ...
    */
    ...
    function hashHandler() {
        ...
        debug("Hash change! New hash: ", currHash, " saved: ", savedHash);
        debug("Starting page: ", startNum);
        savedHash = currHash;
    ...
    function addJailScores(ev=null, retries=0) {
        ...
        if (elemList.length == 0) {
            ...
            return setTimeout(addJailScores, 500, ev, retries++);
        }
        ...
        debug("[addJailScores] scores calculated: ", $(".score").length);
        scoresCalculated = true;

        if (ev) {
            debug("[addJailScores] calling sort");
            handleTitleClick(ev);
        }
    }
    ...
    function handleTitleClick(ev, retries=0) {
        ...
        if (!$(`.${ID}`).length) {
            debug(`...need ${ID}, try adding scores again: `, retries);
            if (retries++ < 30) {
                return setTimeout(addJailScores, 250, ev);
                //setTimeout(handleTitleClick, 250, ev, retries);
            }
            return debug("Didn't find class '", ID, "', timed out.");
        }
    ...
    function queryPastBusts(queryStats=true) {
        ...
        // "https://api.torn.com/user/?comment=XedX Jail Scores v2.0&selections=timestamp,log&log=5360&key="
    }
                            ...
                            if (text.toLowerCase().indexOf('you busted') > -1) {
                                ...
                                if (isToday() == false) {
                                    ...
                                    if (currBustsToday != (+bustsCounted + 1)) {
                                        debug("*** daily busts: error? ", currBustsToday, bustsCounted);
                                        currBustsToday = bustsCounted;
                                    }
                                }
    ...
    function addUnhideBtn(retries=0) {
        const unhideBtn = `<span id="unhide-btn"></span>`;
        if (!$("#unhide-btn").length) {
            let msgBar = $("#mainContainer > div.content-wrapper > div.msg-info-wrap > div > div > div > div");
            if (!$(msgBar).length) {
                if (retries++ < 20) return setTimeout(addUnhideBtn, 250, retries);
                log("ERROR: addUnhideBtn timed out!");
            }
            $(msgBar).parent().css("display", "flex");
            $(msgBar).after(unhideBtn);
            $("#unhide-btn ").on('click', doShow);
        }
    }
    ...
    function reloadUserList(event) {
        debug("Enter reloadUserList, forced: ", forceReloadPageNum, " doingReload: ", doingReload);
        ...
        if (doingReload) {
            doingReload = false;
            debug("Exit reload: already in reload?");
            return;
        }
        ...
        if (h) {
            hashNum = h.split('=')[1];
            hash = h;
            debug("Forcing hash, going to: ", hash, (reloadURL + hash));
        }
    ...
    function processResponse(response, pageNum) {
        ...
        if (jsonObj.success == true) {
            ...
            // Handle the case of no one in jail
             if (!players) {
                ...
                $(playersInJailSel).text("0");      // TBD: change text also
                 ...
                 debug("[processResponse] exit, no players");
                 return;
            }
            ...
            debug("last: ", $(last));
            debug("last pg num: ", lastPgNum, " this page: ", thisPageNum,
                " reloadPage: ", reloadPage, "\nHash: ", location.hash);

            if (!$(tmpDiv).length) {
                debug("Removing pagination");
            ...
            if ((!$(tmpDiv).length || parseInt(thisPageNum) > parseInt(lastPgNum)) && location.hash && location.hash.indexOf('start=') > -1) {
                debug("Reload first page!!", thisPageNum);
                //debugger;
        ...
        debug("[processResponse] Successful Update!");
        doingReload = false;
        observerOn();
    }
    ...
    function getDailyBusts(daysAgo=0) {
        ...
        xedx_TornUserQuery(null, queryStr, dailyBustsCB, daysAgo);
    ...
    function addOptsTableStyles() {
        GM_addStyle(`
            ...
            #xedx-jail-opts-tbl tr.cthree td {
                min-width: 30%;
            }
    ...
    function contentLoadHandler() {
        ...
        callOnContentComplete(doReloadPageSort);
    }
