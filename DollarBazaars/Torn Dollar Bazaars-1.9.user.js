...
// @version      1.9
// @description  This script jumps to the $1 bazaars, in order, and notifies you if anything is available
// @author       xedx [2100735]
...
(function() {
    ...
    function toggleHidden(force) {
        log("ToggleClass: ", force);
        $("#dollarDiv").toggleClass("noevents", force);
        $("#dollarDiv table").toggleClass("hidden", force);
        if (!$('#dollarDiv table').hasClass("hidden")) {
            log("Showing table: ", $('#dollarDiv table'));
            $("#cls-btn").text("Close");
        } else {
            log("Hiding table: ", $('#dollarDiv table'));
            $("#cls-btn").text("Open");
        }
    }
    ...
    */
    ...
    function scrollUpToNode(elementA, elementB) {
        //var elementA = $('#elementA');
        //var elementB = $('#elementB');

        // Calculate the target scroll position
        var targetScrollPosition = elementB.offset().top + elementB.outerHeight();

        log("[scrollUpToNode] ", $(elementA), $(elementB));
        log("tsp: ", targetScrollPosition, " top: ", elementB.offset().top, " h: ", elementB.outerHeight());

        // Animate the scroll
        //$('html, body').animate({
        $(elementA).closest(".editor-content").animate({
        //$(elementA).animate({
    ...
    function handleCloneNewBtns(e) {
        ...
        if (!$(orgItem).length) {
            let itemLoc = $(clone).attr("data-itemLoc");
            let item = clonedItems[itemLoc];
            log("Missing org item. itemLoc: ", itemLoc, " item: ", item, $(item));
            if ($(item).length) {
                log("Found item with location!");
                orgItem = $(item);
            } else {
            ...
            if (!$('.dollar-clone').length) { // No more nodes, hide table
                $("#dollarDiv table").addClass("hidden");
            }
            return;
        }
        ...
        if (isCtrlPanel == true && btnIdx == 0) {
            ...
            //log("call scrollUpToNode");
            //scrollUpToNode($(orgItem), $("#dollarDiv table"));
        } else {
    ...
    function handleCloneClick(e) {  // Not working 100%, z-index issues...
        e.stopPropagation();
        e.preventDefault();
    ...
    function getDollarDiv() {
        ...
        let div = `
            <div id="dollarDiv" class="noevents">
                <table class="hidden">
                    <tbody>
                        <div>
                            ${newRow()}
                            <div id='xtab-left'>
                                <span id="next-btn" class="tab-btn2">
                                    Next
                                </span>
                                <span id="cls-btn" class="tab-btn2">
                                    Open
                                </span>
                            </div>
                        </div>
    ...
    function insertCell(cell) {
        ...
        toggleHidden(false);
        //$("#dollarDiv table").removeClass("hidden");
    ...
    function handleUnlockedItems() {
        for (let idx=0; idx<$(unlockedDollarItems).length; idx++) {
            ...
            if ($(item).attr('data-cloned') == "true" || $(item).hasClass("cloned")) {
                debug("Failed ests: ", ($(item).attr('data-cloned') == "true"),  $(item).hasClass("cloned"));
                continue;
            }
            if (clonedItems[itemLoc] != undefined) {
                debug("Failed item loc test: ", itemLoc, clonedItems[itemLoc]);
                continue;
            }

            $(item).addClass("cloned");
            $(item).attr('data-cloned', "true");
            clonedItems[itemLoc] = $(item);

            let cloned = $(item).clone(true, true);
            let id = 'cloned-' + clonedCnt;
            let itemId = 'org-' + clonedCnt;
            $(cloned).attr("id", id);
            $(cloned).attr('data-itemLoc', itemLoc);
            $(item).attr('id', itemId);
            $(item).css("opacity", 0.1);    // Set visibility to "hidden" or opacity to 0?
            clonedCnt++;

            debug("cloned item: ", $(cloned));
            debug("Item attr: ", $(item).attr('data-cloned'));
            debug("Item class: ", $(item).attr('class'));
            debug("Item loc: ", itemLoc, " index: ", $(item).index(), " parent row: ", $(item).parent().parent().index());
            //debug("cloned items: ", clonedItems);
            insertCell($(cloned));
    ...
    */
    ...
    function clickCostBtn(retries=0) {
        if (retries == 0) log("[clickCostBtn]");
        if (costBtnClicked == true) return;
        let costBtn = $("[class^='searchBar_']").find("[aria-label='Search bar button: Cost']");
        if (!$(costBtn).length) {
            costBtn = $("#bazaarRoot > div > div[class*='searchBar_'] > button:nth-child(5)");
            if (!$(costBtn).length) {
                if (retries++ < 50) return setTimeout(clickCostBtn, 250, retries);
                return log("clickCostBtn timed out.");
            }
        }
        log("Clicking ", $(costBtn));
        $(costBtn)[0].click();
        costBtnClicked = true;
    }

    function addScrollWrap(retries=0) {
        log("[addScrollWrap] Looking for search bar...");
        let search = $("[class*='searchBar_']");
        if (!$(search).length) {
            if (retries++ < 30) return setTimeout(addScrollWrap, 250, retries);
            return log("[addScrollWrap] timed out");
        }

        log("[addScrollWrap] Search bar: ", $(search));

        //let content = $(search).next();
        let scrollWrap =
            `<div id='scroll-outer' class='hdr-wrap'>
            </div>`;

        $(search).wrap(scrollWrap);

        log("[addScrollWrap] Wrapped: ", $("#scroll-outer"));

        if (false) {
            log("[addScrollWrap] Content: ", $(content));
            $("#scroll-outer").append($(`<div id='scroll-content'></div>`));
            $("#scroll-content").append($(content));
            log("[addScrollWrap] scroll-content: ", $("#scroll-content"));

            $('#scroll-content').on('click', function(event) {
                event.preventDefault(); // Prevent default click action on outer div
                event.stopPropagation(); // Stop event from bubbling up to parent elements
            });
        }

        var element = $("#dollarDiv > table > tbody > tr:first-child"); // Replace with your element's selector
        var bottomEdgeInViewport = element.offset().top + element.outerHeight(true) - $(window).scrollTop();

        log("bottomEdgeInViewport: ", bottomEdgeInViewport);
        log("search element: ", search.offset().top, $(window).scrollTop());

        //$(".ReactVirtualized__Grid__innerScrollContainer").addClass("cust-scroll");

        var prevSPos = $(element).css("position");
        $(window).on('scroll', function() {
            var element = $("[class*='searchBar_']"); // Replace with your element's selector
            var scrollTop = $(window).scrollTop();
            var threshold = 617; //bottomEdgeInViewport; //200;

            log("scrollTop: ", scrollTop, " th: ", threshold);

            if (scrollTop > threshold) {
                log("Hit threshold, pos: ", $(element));
                element.css({
                  'position': 'fixed',
                  'top': (bottomEdgeInViewport + 'px'),  //'0px', // Adjust as needed
                  'border': '1px solid limegreen',
                  'z-index': 8
                });
                log("Hit threshold...", $(element));
            }
            /*else {
                element.css({
                  'position': 'static', //prevSPos, //'static', // Or 'relative' if that's its original position
                  'top': 'auto'
                });
            }*/

            if (false) {
                element = $(".ReactVirtualized__Grid__innerScrollContainer");
                threshold = 610;
                if (scrollTop > threshold) {
                    element.css({
                      'position': 'fixed',
                      'top': '32px', //(bottomEdgeInViewport + 'px'),  //'0px', // Adjust as needed
                ...
                /* else {
                    element.css({
                      'position': 'relative', //'static', // Or 'relative' if that's its original position
                      'top': '0px'
                    });
                }*/
            }
    ...
    function addScrollWrap2(retries=0) {
    ...
    function handleUserPageLoad(retries=0) {
        ...
        debug("[handleUserPageLoad] retries: ", retries);
        $(target).css("margin-top", "0px");
        $(target).before(getDollarDiv());
        elementOffset = $('#dollarDiv').position().top - 80;
        $('#dollarDiv').addClass("sticky");

        $("#next-btn").on('click', handleNextBazaar);
        $("#cls-btn").on('click', function() {
            toggleHidden();
        });

        debug("Call addScrollWrap? ", enableScrollLock);
        if (enableScrollLock == true) {
            addScrollWrap();
        }

        debug("Starting orgItemObserver: ", $(target));
    ...
    function addStyles() {
        ...
        GM_addStyle(`
            ...
            .sticky .tt-sub-vendor-highlight {
                background-color: rgba(0, 165, 0, 0.4) !important;
            }
            .dollar-clone {
                background: rgba(0, 165, 0, 0.4) !important;
            }
            ...
            td .dollar-clone.tt-sub-vendor-highlight {
            ...
            .tr-next, .xxxhidden {display: none;}
