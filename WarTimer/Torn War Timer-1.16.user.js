...
(function() {
    ...
    function rwReqCb(responseText, ID, options) {
        ...
        try {
            ...
            console.error("Torn War Timer parse error: ", e, "\nresponse: ", responseText);
            debugger;
            return;
        }
        if (jsonObj.error) {
    ...
    function handlePageLoad(retries=0) {
        ...
        let pageOk = false;
        if (params.get('step') == 'your') pageOk = true;
        if (params.get('step') == 'profile' && params.get('ID') == myFacId) pageOk = true;
