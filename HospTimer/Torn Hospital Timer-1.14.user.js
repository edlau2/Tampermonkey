...
(function() {
    ...
    var savedPos;
    GM_addStyle(`.xsc { transform: scale(2.0) !important; }`);
    function handleMinimizeClick() {
        GM_setValue("lastDisplaySize", "32px");
        let wrapper = $($("[class^='group-chat-box_']")[0]);
        if (!$(wrapper).length) {
            log("Minimize error: no wrapper!");

            //$("#x-hosp-watch").toggleClass("xsc x-hosp-wrap");
        ...
        //$("#x-hosp-watch").detach().prependTo($("#chatRoot"));
