...
(function() {
    ...
    if (!singleton) {
      ...
      class FFScouterCache {
            ...
            for (const player_id of player_ids) {
              ...
              promises.push(
                new Promise((resolve, reject) => {
                  res.onerror = () => {
                      debug(`get player ${player_id} failed: ${res.error}`);
                    reject(res.error);
                  };

                  res.onsuccess = () => {
                    if (res.result && res.result.expiry > Date.now()) {
                        debug(`get player ${player_id} success: ${res.result}`);
        ...
        delete_db = async () => {
          return new Promise((resolve, reject) => {
            ...
            r.onerror = () => {
              debug(`Error deleting indexedDB (${this.db_name}): ${r.error}`);
              reject(r.error);
            };

            r.onsuccess = () => {
              debug(`Successfully deleted indexedDB (${this.db_name})`);
              resolve(r.result);
            };
          });
        };
      ...
      if (!rD_getValue(CLEARED_TSC_KEY)) {
        log("Trying to delete any TSC keys found");
        ...
        log(`Found ${badkeys.length} TSC keys`);
      ...
      function ffdebug(...args) {
          if (debugLoggingEnabled != true) return;
          debug(...args);
      ...
      async function process_queue() {
        if (queued_player_ids.length > 0) {
          ...
          await process_queued_player_ids(processing_player_ids, function () {
              debug("[process_queued_player_ids]");
      ...
      async function process_queued_player_ids(player_ids, callback) {
        ...
        if (unknown_player_ids.length > 0) {
                ...
                debug("Res: ", response);
                  ...
                  if (response.status == 200) {
                    ...
                    try {
                      ...
                      if (err && err.error) {
                          debug("API req failed: ", err.error, err.code);
                        showToast(
                          ...
                          debug("API request failed. HTTP status code: ", response.status);
                        showToast(
          ...
          rD_xmlhttpRequest({
            ...
            debug("Calling cb for ", player_ids);
          callback(player_ids);
        }
      }
