...
(async function () {
  "use strict";
    ...
    console.log("FF Scouter calling logScriptStart");
    logScriptStart();
    console.log("FF Scouter called logScriptStart");
    ...
    if (!singleton) {
      ...
      GM_addStyle(`
        ...
        function ff2_debounce(fn, delay) {
          let timer = null;
        ...
        function ff2_has_est_header() {
          return !!document.querySelector(".ff-scouter-est-header");
        }

        function ff2_ensure_est_header() {
          if (ff2_has_est_header()) return;

          const lvl = document.querySelector(".table-header > .lvl");
          if (!lvl) return;
          ...
          est.classList.add(
            ...
            "c-pointer",
            "ff-scouter-est-visible",
            "ff-scouter-est-header"
          );

          est.onclick = () => sortByEstimate();
          est.textContent = "Est";

          lvl.after(est);
        }

        function ff2_row_has_est(row) {
          return !!row.querySelector(".ff-scouter-est-cell");
        }

        // prevent duplicate API bursts
        const ff2_pending = new Set();
      ...
      async function update_ff_cache(player_ids, callback) {
        if (!key) {
          ...
          player_ids = player_ids.filter(id => !ff2_pending.has(id));

        clean_expired_data();

        var unknown_player_ids = await get_cache_misses(player_ids);
          unknown_player_ids.forEach(id => ff2_pending.add(id));
      ...
      setTimeout(process_queue, 10);
        ...
        // TEMP disable...maybe not doing async?
            ...
            onload: function (response) {
              ...
              if (response.status == 200) {
                ...
                ffcache.update_cache(cachedObjs).then(() => {
                  player_ids.forEach(id => ff2_pending.delete(id));
                  callback(player_ids);
                });
      ...
      function get_contrast_color(hex) {
        ...
        function valFromHr(txt) {
            if (!txt) return debug("valFromHr, no param!");

            if (txt.indexOf('k') > -1)
                txt = parseInt(txt) * 1000;
            else if (txt.indexOf('m') > -1)
                txt = parseInt(txt) * 1000000;
            else if (txt.indexOf('b') > -1)
                txt = parseInt(txt) * 1000000000;
        ...
        function sortByEstimate() {
            debug("sortByEstimate] sortByEstimate: ", lastOrder);
            let list = $(".faction-info-wrap.another-faction ul.table-body");
            const items = Array.from($(list).find('li.table-row'));

            items.sort((a, b) => {
                // Get the value from the child element's data attribute
                let aValue = parseFloat($(a).attr('data-est'));
                let bValue = parseFloat($(b).attr('data-est'));
                if (!aValue || isNaN(aValue)) {
                    let div = $(a).find("div.ff-scouter-est-visible");
                    let txt = $(div).text();
                    aValue = valFromHr(txt);
                    if (!isNaN(aValue)) $(a).attr("data-est", aValue);
                }
                 if (!bValue || isNaN(bValue)) {
                    let div = $(b).find("div.ff-scouter-est-visible");
                    let txt = $(div).text();
                    bValue = valFromHr(txt);
                    if (!isNaN(bValue)) $(b).attr("data-est", bValue);
                }

                return lastOrder ? (aValue - bValue) : (bValue - aValue);
            });

            // 3. Reorder the list items in the DOM
            // Appending an existing child node moves it from its current position
            items.forEach(item => {
                $(list).append(item);
            });
      ...
      async function apply_fair_fight_info(_) {
        ...
        //   });
        // };
        // ff_li.appendChild(document.createTextNode("Est"));
    ...
    //     est_li.classList.add("ff-scouter-est-visible");

    //     est_li.onclick = () => {
    //       debug("Est column click, do sort");
    //         sortByEstimate();
          ...
          //    debug("Est column click 2, do nothing? Or attack");
        ...
        if ($(".table-header > .lvl").length == 0) return;
        ff2_ensure_est_header();
        ...
        //$(".table-body > .table-row > .member").each(async function (_, value) {
          $(".table-body > .table-row").each(async function (_, row) {
              if (ff2_row_has_est(row)) return;

              const value = row.querySelector(".member");
              if (!value) return;
              var url = value.querySelectorAll('a[href^="/profiles"]')[0].href;
       ...
       // $(".table-body > .table-row > .member").each(async function (_, value) {
          ...
          estimate_div.classList.add("ff-scouter-est-visible");
          estimate_div.classList.add("ff-scouter-est-cell");
    ...
    //         fair_fight_div.innerHTML = ff_string;
            ...
            if (cached.bs_estimate_human) {
              ...
              //debug("Add est to li: ", $(estimate_div).parent(), cached.bs_estimate);
              $(estimate_div).parent().attr("data-est", cached.bs_estimate);
          }
    ...
    /*
      ...
      if (match) {
        ...
        if (!key) {
          ...
          let ff2_membersObserver = null;

          const scan = ff2_debounce(async function () {
            const list = document.querySelector(".members-list");
            if (!list) return;

            const rows = list.querySelectorAll(".table-body > .table-row");

            let needs = false;
            for (const r of rows) {
              if (!ff2_row_has_est(r)) {
                needs = true;
                break;
              }
            }
            ...
            const ids = get_members();
            await update_ff_cache(ids, apply_fair_fight_info);

          }, 200);

          const wait = new MutationObserver(ff2_debounce(function () {
            const list = document.querySelector(".members-list");
            if (!list) return;

            wait.disconnect();

            if (!list.__ff2Observed) {
              list.__ff2Observed = true;

              ff2_membersObserver = new MutationObserver(function (mutations) {
                let relevant = false;

                for (const m of mutations) {
                  for (const n of m.addedNodes) {
                    if (!(n instanceof Element)) continue;
                    if (n.matches?.(".table-row") || n.querySelector?.(".table-row")) {
                      relevant = true;
                      break;
                    }
                  }
              ...
              ff2_membersObserver.observe(list, { childList: true, subtree: true });
            }
      ...
      // ===============================================================
        ...
        const match = match1 ?? match2;
        const isFactionPage = window.location.href.startsWith("https://www.torn.com/factions.php");
      ...
      async function show_cached_values(elements) {
        ...
        for (const [player_id, element] of elements) {
          ...
          if (!element.classList.contains("indicator-lines")) {
            ...
            element.style.setProperty("--arrow-width", "12px");
          ...
          const ff_gauge_observer = new MutationObserver(async function () {
            ...
            if (honor_bars.length > 0) {
              ...
              if (window.location.href.startsWith("https://www.torn.com/companies.php")) {
                ...
                window.location.href.startsWith("https://www.torn.com/page.php?sid=competition#/team")
              ) {
              ...
              } else if (window.location.href.startsWith("https://www.torn.com/loader.php?sid=attackLog")) {
            ...
            if (window.location.href.startsWith("https://www.torn.com/page.php?sid=ItemMarket")) {
              await apply_ff_gauge(
                $("div.bazaar-listing-card div:first-child div:first-child > a").toArray()
              );
            }
