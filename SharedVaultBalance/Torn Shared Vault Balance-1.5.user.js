...
(function() {
    ...
    function logQueryCB(responseText, ID, param) {
        ...
        debug("***** initStartDate: ", initStartDate, " oldestDate: ", oldestDate, " mostRecent: ", mostRecent, " *****");
        if (initStartDate && initStartDate < oldestDate && inResetAll == true) {
            if (count++ < 4) { // safety net
                // recurse
                let oldDate = new Date(Number(oldestDate) * 1000);
                debug("***** Getting next log entries prior to ", oldDate, " *****");
