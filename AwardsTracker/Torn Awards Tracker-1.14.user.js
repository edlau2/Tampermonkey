...
// @version      1.14
// @description  View locked awards on one page, use the Legacy merits page
// @author       xedx [2100735]
...
// Add a tab listing unowned ('locked') honors and medals
// API key required: limited
//
// API calls: USER honors, medals - owned awards
//            TORN honors, medals - All available awards
...
(function() {
    ...
    // May need to update dynamically? The class names may change...
    ...
    */
    ...
    const lgcyVwTxt = "Legacy View";
    const stdVwTxt = "Standard View";

    const p = `<div class="pip"></div>`;
    const po = `<div class="pip owned"></div>`;
    const pips0 = p + p + p + p + p + p + p + p + p + p;
    ...
    const meritBars = [pips0, pips1, pips2, pips3, pips4, pips5, pips6, pips7, pips8, pips9, pips10];

    var availMerits = [];
    var meritsToSpend;
    var meritsSpent;

    function getCloneMeritsToSpend() { return $($("[class^='meritsValue'] > p > [class^='violet']")[0]).text(); }
    function getCloneMeritsSpent() { return $($("[class^='meritsValue'] > p > [class^='violet']")[1]).text(); }
    function getActualMeritsToSpend() { return $($("[class^='meritsValue'] > p > [class^='violet']")[2]).text(); }
    function getActualMeritsSpent() { return $($("[class^='meritsValue'] > p > [class^='violet']")[3]).text(); }

    // All the things you can  buy with merits. So an 'available merit' is
    // something like {name: "Brawn", desc: "Gives a passive...", owned: 5}
    function getAvailMerits() {
        if (availMerits.length > 0) return;
        let mlist = $("#merits-panel > ul > li [class^='meritInfo']");
        for (let idx=0; idx<$(mlist).length; idx++) {
            let node = mlist[idx];
            let root = $(node).closest('li');
            let nameNodes = $(node).find("p[class^='title'] > span");
            let name = $($(nameNodes)[0]).text();
            let owned = parseInt($($(nameNodes)[1]).text().split('/')[0]);
            let desc = $(node).find("p[class^='description']").text();
            availMerits.push({title: name, desc: desc, owned: owned})
        }
        log("availMerits: ", availMerits);
    }

    // Get this class name dynamically...it may change...
    ...
    function toggleLegacy(e) {
        ...
        //log("[toggleLegacy]: '", $(".xlegacy-btn").text().trim(), "', '", $(this).text().trim(), "', ",
        //    ($(".xlegacy-btn").text() == lgcyVwTxt), ($(".xlegacy-btn").text().indexOf('Legacy') > -1));

        if ($(".xlegacy-btn").text().indexOf('Legacy') > -1)
            $(".xlegacy-btn").text(stdVwTxt);
        else
            $(".xlegacy-btn").text(lgcyVwTxt);
    }

    var waitingOnAfterSpendDlg = false;
    function handleMeritRowClick(e) {
        // Get the actual target on non-legacy panel
        let rowIdx = parseInt($(this).attr("data-index"));
        let mlist = $("#merits-panel > ul > li");
        let target = $(mlist)[rowIdx];
        $(".xRow").removeClass("xRow");

        log("dlgClone: ", $('.dlgClone'));

        $('.dlgClone').remove();

        // This will open a new dialog with "SPEND, Cancel" or "Close"
        log("clicking actual target: ", $(target));
        let thisRow = $(this);
        $(thisRow).addClass("xRow");
        $(target).trigger('click');

        // Delay slightly for existing dialogs to close?
        setTimeout(waitOnDlg, 250, $(thisRow));

        // Wait for the dialog to appear, to clone
        function waitOnDlg(row, retries=0) {
            let dlg = $("#merits-panel > div[class^='dialogWrap_']");
            log("[waitOnDlg] ", retries, $(row), $(dlg));
            if (!$(dlg).length) {
                if (retries++ < 20) return setTimeout(waitOnDlg, 100, row, retries);
                return log("[waitOnDlg] timed out");
            }

            // Handlers not copied? Add our own, forward to original
            // dialogWrap > .action_ > button, text = 'Close', 'SPEND', 'Cancel'
            //
            let dlgClone = $(dlg).clone(true, true);
            $(dlgClone).addClass('dlgClone');
            $(dlg).addClass("stdDlg");
            $(row).after($(dlgClone));

            $(".dlgClone").find("button").on('click', handleCloneBtn);
        }

        // Temporary - see what happens when "SPEND" is clicked...
        function handleSpendButton(dlgClone, row) {
            log("[handleSpendButton] clone: ", $(dlgClone), " row: ", $(row));

            log("SPEND clicked, new dlg: ", $("#merits-panel > div[class^='dialogWrap_']"));
            log("merits to spend, before: ", meritsToSpend, " now: ", getActualMeritsToSpend());
            log("merits spent, before: ", meritsSpent, " now: ", getActualMeritsSpent());

            // Maybe need to wait for new, updated dialog...check in observer...
            if (parseInt(getActualMeritsToSpend()) > parseInt(meritsToSpend)) {
                $($("[class^='meritsValue'] > p > [class^='violet']")[0]).text(getActualMeritsToSpend());
                $($("[class^='meritsValue'] > p > [class^='violet']")[1]).text(getActualMeritsSpent());
            }

            // This doesn't seem to work as planned. The observer does catch mutations...
            // Seems the dialog has several alternate texts? upgradeInfo, confirmedInfo, unavailableInfo
            // p nodes?

            // Wait for observer to catch all changes first !!!!!

            //$(dlgClone).remove();
            log("clone (this): ", $(".dlgClone"));
            log("std dlg: ", $(".stdDlg"));
            log("Row: ", $(".xRow"));

            let newDlg = $(".stdDlg").length > 0 ? $(".stdDlg") : $("#merits-panel > div[class^='dialogWrap_']");

            let currClone = $(".dlgClone");
            $(currClone).css("border", "1px solid green");

            waitingOnAfterSpendDlg = true;

            /*
            log("New dlg: ", $(newDlg), " org clone: ", $(".dlgClone"));

            let newClone = $(newDlg).clone(true, true);
            log("Adding new clone: ", $(newClone));
            $(row).after($(newClone));

            log("Remove curr clone: ", $(currClone));
            $(currClone).remove();
            */
        ...
        // Handle btn click on original dlg
        // btn is orig btn, text is bn command - SPEND, Cancel, Close
        function handleBtnTriggered(btn, txt, retries=0) {
            let dlg = $("#merits-panel > div[class^='dialogWrap_']");
            log("[handleBtnTriggered]: ", txt, retries, $(dlg).length, $(dlg));

            // On close/cancel, already removed!! (?)
            let dlgClone = $(btn).closest("div[class^='dialogWrap_']");
            let row = $(".xRow");
            log("dlgClone: ", $(dlgClone), " row: ", $(row));

            if (txt == 'Close' || txt == 'Cancel') {
                if ($(dlg).length > 0) {
                    if (retries++ < 5) return setTimeout(handleBtnTriggered, 100, btn, txt, retries);
                    log("[handleBtnTriggered] too many retries! ", retries, $(dlg));
                    log("Manually remove?");
                }
            } else if (txt == 'SPEND') {
                meritsToSpend = getCloneMeritsToSpend();
                meritsSpent = getCloneMeritsSpent();
                setTimeout(handleSpendButton, 250);
            } else {
                log("ERROR: unknown btn: ", txt, $(btn).text(), $(btn));
            }
        }

        // Forward a click on the cloned legacy element to the real one
        // Result of that action is handled above in handleBtnTriggered
        function handleCloneBtn(e) {
            log("[handleCloneBtn] ", $(this).text(), $(this));

            let txt = $(this).text();
            let orgBtns = $("#merits-panel > div[class^='dialogWrap_'] button");

            log("org btns: ", $(orgBtns));

            if (txt == 'Close' || txt == 'Cancel') {
                log("Closing clone: ", $(this).closest("div[class^='dialogWrap_']"));
                $(this).closest("div[class^='dialogWrap_']").remove();
            }

            for (let idx=0; idx<$(orgBtns).length; idx++) {
                let btn = $(orgBtns)[idx];
                if ($(btn).text() == txt) {
                    log("Found btn: ", $(btn).text(), $(btn), $(btn)[0].onclick);

                    let dlg = $("#merits-panel > div[class^='dialogWrap_']");
                    log("dlg: ", $(dlg));
                    $(btn).trigger('click');

                    // Handle new dialog, or make sure cancelled/closed
                    handleBtnTriggered($(btn), txt);
                }
            }
        }
    }

    function handleSpendDlgWait(dlg) {
        log("[handleSpendDlgWait] ", $(dlg));
        cbTimer = null;

        let row = $(".xRow");
        let currClone = $(".dlgClone");
        let stdDlg = $(".stdDlg");
        log("[handleSpendDlgWait] Std dlg: ", $(stdDlg), " dlg: ", $(dlg));
        log("[handleSpendDlgWait] Curr clone: ", $(currClone));
        log("[handleSpendDlgWait] std, by sel: ", $("#merits-panel > div[class^='dialogWrap_']"));

        log("[handleSpendDlgWait] Remove curr clone: ", $(currClone));
        $(currClone).remove();

        let newClone = $(dlg).clone(true, true);
        $(newClone).addClass("dlgClone").removeClass("stdDlg");
        log("[handleSpendDlgWait] Adding new clone: ", $(newClone));
        $(row).after($(newClone));

        waitingOnAfterSpendDlg = false;

        log("[handleSpendDlgWait] clone: ", $(".dlgClone"), " std dlg: ", $(".stdDlg"));
    }

    /*
    If waitingOnAfter spend: look in nodes added for classes:
     class:  upgradeInfo___Zo0Ej unavailableInfo___ahXPh |, use indexOf
     set timer ... say 250. When adedNodes hit again, reset timer if it hasn't been cleared.
     So hopefully when done chaging the std dlg, we can  then clone....
    ...
    */
    ...
    var obsTarget;
    var cbTimer;
    const config = { childList: true, subtree: true, characterData: true, characterDataOldValue: true, attributes: true };
    function installObserver(retries=0) {
        obsTarget = $("#merits-panel");
        debug("[installObserver] ", $(obsTarget));
        if (!$(obsTarget).length) {
            ...
            for (const mutation of mutationsList) {
                if (mutation.type === 'childList') {
                    if (mutation.addedNodes.length > 0) {
                        debug("[handleMutations]Added: ", mutation.addedNodes);

                        for (let idx=0; idx<mutation.addedNodes.length; idx++) {
                            let newNode = mutation.addedNodes[idx];
                            debug("[handleMutations] adding ", $(newNode));
                            let cl = $(newNode).attr("class");
                            log("class: ", cl);
                            let parent = $(newNode).parent();
                            let parentCl = $(parent).attr("class");
                            log("Parent: ", $(parent), " parent class: ", parentCl);
                            log("Prev sib: ", $(mutation.previousSibling), mutation.previousSibling);

                            if (waitingOnAfterSpendDlg == true && $(parent).hasClass("stdDlg")) {
                                if (cbTimer) clearTimeout(cbTimer);
                                log("Setting spen-wait timer with ", $(parent));
                                cbTimer = setTimeout(handleSpendDlgWait, 250, $(parent));
                            }
                        }
                    }
                    if (mutation.removedNodes.length > 0) {
                        debug("[handleMutations] Removed: ", mutation.removedNodes);

                        for (let idx=0; idx<mutation.removedNodes.length; idx++) {
                            let oldNode = mutation.removedNodes[idx];
                            debug("[handleMutations] removing ", $(oldNode));
                            log("class: ", $(oldNode).attr("class"));
                        }
                    }
                }
                if (mutation.type == 'characterData') {
                    log("[handleMutations] characterData");
                    log("[handleMutations] node: ", mutation.target);
                    log("[handleMutations] oldValue: ", mutation.oldValue);
                }
            }
        };

    observer = new MutationObserver(handleMutations);
    observer.observe($(obsTarget)[0], config);
    }

    function observerOn() { observer.observe($(obsTarget)[0], config); }
    function observerOff() { observer.disconnect(); }
    ...
    function installMeritsUi(retries=0) {
        ...
        let btnRow = `<div class="btn-wrap silver c-pointer" style="margin-left: auto;">
                          <a class="xlegacy-btn btn torn-btn btn-dark-bg">${lgcyVwTxt}</a>
                      </div>
                      `;
        $(hdr).append($(btnRow));
        installMeritsUiPage();
    ...
    function installMeritsUiPage() {
        ...
        let pageParent = $("#merits-panel").parent();
        let legacyPage = getLegacyPage();
        ...
        $(".merit-row").on('click', handleMeritRowClick);
        ...
        function addLegacyRows() {
            availMerits.forEach((entry, index) => {
                let idx = Number(entry.owned);
                let row = `
                    <li tabindex="0" data-index="${index}" class="merit-row" aria-label="Merit: ${entry.title}">
                        <span class="xdivider">
                            <span class="xmtitle">${entry.title}</span><span class="xmdesc"> - ${entry.desc}</span>
                        </span>
                        <span class="xdesc">
                            <div class="bar-wrap">
                                ${meritBars[idx]}
                ...
                if (index == 11 || index == 15)
                    $("#legacy-merits > ul").append(sectionDivider);
            });
        }
    ...
    GM_addStyle(`.toGoTip { font-size: 12px !important; color: limegreen !important; }`);
    function processLocked(id, retries=0) {
        // $("#honors-panel > ul:nth-child(3) > li:nth-child(1) > div").text()
        let locked = $("#honors-panel > ul > li > div[class*='lockedHonor']");
        debug("[processLocked] locked: ", $(locked));
        if (!$(locked).length) {
            if (retries++ < 20) return setTimeout(processLocked, 250, id, retries);
            return log("Error: processLocked timed out.");
        }
        for (let idx=0; idx<$(locked).length; idx++) {
            let node = $(locked)[idx];
            let prog = $(node).text();
            let name = $(node).closest('li').attr("aria-label");
            debug("Honor: ", name, " progress: ", prog);
            if (prog) {
                let parts = prog.split('/');
                if (parts && parts.length > 1) {
                    let have = parseInt(parts[0].replaceAll(',', ''));
                    let need = parseInt(parts[1].replaceAll(',', ''));
                    if (have && need && !isNaN(have) && !isNaN(need)) {
                        let left = need - have;
                        //let txt = parts[0] + ' / ' + parts[1] + " <---> " + numberWithCommas(left) + " to go";
                        let txt = numberWithCommas(left) + " to go!";
                        //$(node).text(txt);
                        displayHtmlToolTip($(node), txt, "tooltip4 toGoTip");
                    }
                }
            }
        }
    ...
    function installUI(retries=0) {
        ...
        $("#honors-panel [class^='categoryList'] > li").on('click', function(e) {
            ...
            log("on tab click, id: ", id, " tab: ", $(e.currentTarget));
            ...
            if (id == 'missing-honors' || id == 'missing-medals') {
                ...
                setTimeout(processLocked, 250, id);
            }
        });
