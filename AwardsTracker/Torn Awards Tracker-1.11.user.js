...
(function() {
    ...
    */
    ...
    var availMerits = []; //JSON.parse(GM_getValue("availMerits", JSON.stringify([])));
    ...
    function handleMeritRowClick(e) {
        ...
        function waitOnDlg(row, retries=0) {
            ...
            if ($(dlg).hasClass('xfound')) {
                log("On old dlg still? ", retries, $(dlg));
                return setTimeout(waitOnDlg, 100, retries++);
            }
            $(dlg).addClass('xfound');
        ...
        function handleCloneBtn(e) {
            ...
            for (let idx=0; idx<$(orgBtns).length; idx++) {
                let btn = $(orgBtns)[idx];
                if ($(btn).text() == txt) {
                    log("Found btn: ", $(btn), $(btn)[0].onclick);
