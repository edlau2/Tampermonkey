...
(function() {
    ...
    function setBestCarSelection(retries=0) {
        ...
        //$(hdr).parent().css("border", "1px solid yellow");
    ...
    function redoSort() {
        prevOrder = (prevOrder == 0) ? 1 : 0;
        log("redoSort: ", prevOrder, prevWhat);
        doSort(prevWhat);
    }
    ...
    const filterClasses = "trackhide pwdhide feehide longhide shorthide starttimehide goldhide";
    ...
    function handleRemovePreset(e) {
        ...
        let find = $(opt).val();
        log("[handleRemovePreset]: ", find, $(opt));

        for (let idx=0; idx<tablePresets.length;idx++) {
            let entry = tablePresets[idx];
            log("preset entry: ", entry);
            if (entry.val == find) {
    ...
    function old_archived() {
         function processPageData(data) {
            ...
            var eventsList = $(newPage).find(".events-list");
            let liList = $(newPage).find(".events-list > li")
            log("liList (1): ", $(liList));

            if ($(liList).length > 0) {
                if (!eventListParent)
                    eventListParent = $(eventsList).parent();

                log("eventsList: ", $(eventsList));

                // Before replacing, need event handlers for "join race"..
                // copyHandlers3($(".join > a")[0], $(eventsList).find(".join > a")[0]);
                // didn't work, maybe TM specific? Works from console...

                // Try cloning, maybe keeping orig list detached?
                // $(".events-list > li:nth-child(2) .acc-body .name").text().trim()
                // $(".events-list > li:nth-child(2) .join > a")
                //
                // cloning also not working. So, just replace the start time and number of
                // drivers, and remove those no longer there? Iterate through existing UL,
                // and find in new page. Not found, remove. Found, replace start time and
                // users, maybe just edit node text, don't replace...

                /*
                if (!joinTemplate) {
                    joinTemplate = $(".events-list > li:first-child .join > a").clone(true, true);
                    log("Template: ", $(joinTemplate));
                }
                */

                /*
                // Create a clone of one 'click to join' node. Try one and see if it's
                // the same, if not, iterate and match by name...
                log("list ln: ", $(liList).length, $(liList));
                for (let idx=0; idx< $(liList).length; idx++) {
                    let node = $(liList)[idx];
                    log("idx: ", idx, " node: ", $(node));
                    let dst = $(node).find(".join > a");
                    log("dst: ", $(dst), " new: ", $(joinTemplate));
                    log("dst: ", $(dst).html());
                    log(" new: ", $(joinTemplate).html());
                    //log("HTML: ", $(node).html());
                    if ($(dst).length)
                        $(dst).replaceWith($(joinTemplate).clone());
                }
                */

                $(eventsList).attr("id", "xxx-elist");

                /*
                if (!detachedChild) {
                    log("Detaching ", $(".events-list"));
                    detachedChild = $(".events-list").detach();
                    log("Parent: ", $(eventListParent));
                    $(eventListParent).prepend(eventsList);
                    log("eventsList: ", $(eventsList));
                } else {
                    log("Replacing ", $(".events-list"));
                    $(".events-list").replaceWith(eventsList);
                }
                */
                ...
                // ==========
                log("new events list: ", $("#xxx-elist"));
                log("by class: ", $("ul.events-list"));
                */

                /*
                let liList = $("ul.events-list > li");
                for (let idx=0; idx< $(liList).length; idx++) {
                    let node = $(liList)[idx];
                    log("idx: ", idx, " node: ", $(node));
                    let dst = $(node).find(".join > a");
                ...
                */
                ...
                for (let idx=0; idx< $(liList).length; idx++) {
                    ...
                    //log("HTML: ", $(node).html());
                    if ($(dst).length)
                        $(dst).replaceWith($(joinTemplate).clone(true, true));
                }
                */
                // =========
        ...
        function copyHandlers1(src, dst) {
            var $header = $('.inner th');
            var $fixedHeader = $(".header1 th");

            let allEvents = $(src).data('events');
        ...
        function copyHandlers2(src, dst) {
            $.each($(src).data('events'), function() {
                // iterate registered handler of original
                $.each(this, function() {
                    log("Handler: ", this);
                    $(dst).bind(this.type, this.handler);
                });
            });
        }

        function copyHandlers3(src, dst) {
            // This syntax works in the Chrome console
            // allEvents = $._data($(".join > a")[0], "events")
            // Also works with 'document' for example, and a
            // simple div I dynamically created and added a
            // handler to...

            log("copyHandlers3, src: ", src, " dst: ", dst);
            let allEvents = $._data(src, "events");
            log("allEvents: ", allEvents);

            allEvents.forEach( event => {
                log("event: ", event);
                $.each(event, function() {
                    $(dst).bind(event.type, event.handler);
                });
            });
        }
    }
    // =================== End saved, just in case... =================================
