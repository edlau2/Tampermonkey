...
(function() {
    ...
    const defFilterTable = {
        ...
        misc: {
            ...
            "Sort":
                { enabled: false, type: "custom", filterFn: "customSort", class: '', id: "table-sort", selected: "none"},
        }
    };
            ...
            if (entry.id == 'table-sort') {
                newCell = `
                    <td><label><input id="${entry.id}" class="xcust" type="checkbox" name="${key}">Sort</label>
                        <select class="xmselect" id="${entry.id}-select" name="${key}" data-selected='${entry.selected}'>`;
