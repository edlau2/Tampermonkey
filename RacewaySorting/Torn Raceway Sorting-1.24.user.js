...
(function() {
    ...
    function updateDriversAttr(liList=$(".events-list > li")) {
        $(liList).each(function(idx, li) {
            ...
            if (parts.length > 1) {
                ...
                if (tmp) tmp = tmp.trim();
                log("tmp: ", tmp);
                let maxUsers = parseInt(tmp); //parts[1].replaceAll('\n', '').trim());
                log("max: ", maxUsers, " curr: ", currUsers);
    ...
    // Add right-click context menus to columns with multiple sort options
    function addSortContextMenus() {
    ...
    function addFilterBtn(retries=0, init) {
        ...
        if (!$(btnWrap).length) {
            ...
            let wasInInit = inInit;
            inInit = false;
            return log("[addFilterBtn] timed out!\n",
                      $(".messages-race-wrap.start-race > div.bottom-round > .btn-wrap"),
                      "\ninInit: ", wasInInit, " retries: ", retries);
        }
        inInit = false;
