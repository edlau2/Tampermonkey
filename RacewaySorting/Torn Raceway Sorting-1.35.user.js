...
(function() {
    ...
    function applyAllFilters(forced = false, init) {
        ...
        $(".xmisc").each(function (idx, el) {
            let key = $(this).attr('name');
            let entry = filterTable.misc[key];
            if (!entry) entry = defFilterTable.misc[key];
    ...
    function getFilterOptsDiv() {
        ...
        function addMiscOptionRows(tableDiv) {
            ...
            for (idx=0; idx<keys.length; idx++) {
                ...
                let entry = filterTable.misc[key];
                if (!entry) entry = defFilterTable.misc[key];
                if (!entry) { // check for alias - table changed
                    let altKey = filterAliases[key];
                    if (altKey) {
                        log("*** Found new key! ", key, " => ", altKey);
                        entry = defFilterTable.misc[altKey];
                        // TBD: Fix tables/presets!
                    }
                }
                if (!entry) {
