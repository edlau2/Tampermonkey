   ...
   `);
    ...
    unsafeWindow.fetch = async (...args) => {
        ...
        if (suppressOn == true) {
            ...
            if (url.indexOf(`/chat`) > -1 || url.indexOf(`/sendbird`) > -1) {
                ...
                return Promise.reject(new TypeError(('Fetch to this URL is blocked by ', + GM_info.script.name)));
            }
        }
    ...
    function installUi(retries=0) {
        ...
        $('#xhsc-root').on('click', function (e) {
            let hide = $('#xhsc').hasClass('xs');  // Is showing, need to hide
            ...
            log("Chat suppression toggled, now is ", ((hide == true) ? 'enabled' : 'disabled'));
            return false;
        });
    }
