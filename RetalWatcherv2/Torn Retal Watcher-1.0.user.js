...
(function() {
    ...
    function getAttacks(from) {
        ...
        from = (from && !isNaN(from)) ? (Math.max(+from, fiveMin) * 1000) : (fiveMin * 1000);
        let fromStr = "from=" + from + "&";

        debug("[getAttacks] from: ", from, " (", (new Date(+from)), ")");

        let url = `https://api.torn.com/v2/faction/attacks?filters=incoming&limit=100&sort=DESC&${fromStr}key=n${api_key}`;
