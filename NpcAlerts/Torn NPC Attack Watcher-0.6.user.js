...
(function() {
    ...
    function doTimeUntilFromNow() {
        let now = new Date();
        log("[debugIntTimer] now.hrs: ", now.getUTCHours());
        log("[debugIntTimer] now.mins: ", now.getUTCMinutes());
        log("[debugIntTimer] now.secs: ", now.getUTCSeconds());

        let when = startTimes.startsAt;
        log("[debugIntTimer] when.hrs: ", when.getUTCHours());
        log("[debugIntTimer] when.mins: ", when.getUTCMinutes());
        log("[debugIntTimer] when.secs: ", when.getUTCSeconds());

        if (now.getUTCHours() > when.getUTCHours()) {  // tomorrow, (24:00:00 - now) + when
            startTimes.until.hrs = Math.abs(when.getUTCHours() + (24 - now.getUTCHours()));
            startTimes.until.mins = Math.abs(when.getUTCMinutes() + (60- now.getUTCMinutes()));
            startTimes.until.secs = Math.abs(when.getUTCSeconds() + (60 - now.getUTCSeconds()));
        } else {                                       // today, when - now
            startTimes.until.hrs = Math.abs(when.getUTCHours() - now.getUTCHours() - 1);
            startTimes.until.mins = Math.abs(60 - when.getUTCMinutes() - now.getUTCMinutes() - 1);
            startTimes.until.secs = Math.abs(60 - when.getUTCSeconds() - now.getUTCSeconds());
        }


        startTimes.minutesUntil = startTimes.until.hrs * 60 + startTimes.until.mins;

        log("[debugIntTimer] hrs: ", startTimes.until.hrs,
    ...
    function handleIntTimer() {
        ...
        if (secCounter++ == 30) {
            ...
            log("[handleIntTimer] times: ", startTimes, " minsUntil: ", minsUntil);
            secCounter = 0;
            doLog = true;
        }
    ...
    function animateDone(sel, size) {
        ...
        if (size == 0) {
            ...
            //$("#uber-alert").css("margin-top", "-22px");
        } else {
            //$("#uber-alert").css("margin-top", "0px");
