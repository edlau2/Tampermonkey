...
function getVersion() {
  return 'XANETS_TRADE_HELPER_VERSION_INTERNAL = "' + XANETS_TRADE_HELPER_VERSION_INTERNA/////////////////////////////////////////////////////////////////////////////
// File: service-provider.js.gs
/////////////////////////////////////////////////////////////////////////////
  ...
  for (let i = 0; i < array.length; i++) { // Iterate names we got from trade grid
    ...
    for (let j = 0; j < names.length; j++) { // to compare to all known names. 
      ...
      if (isQuran) { // Handle one of the Quran Scripts
        ...
        if (names[j].toString().trim().includes('Bottle of Sak')) {
          nameFound = true;
        }
      } else if (isGameConsole) {
        if (names[j].toString().trim() == 'Playstation' || names[j].toString().trim() == 'Game Console') {
          nameFound = true;
        }
      } else if (searchWord == names[j].toString().trim()) { // Handle everything else
          nameFound = true;
      }
