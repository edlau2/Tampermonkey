...
(function() {
    ...
    function processLoadResponse(response, status, xhr, target) {
        ...
        debug("[processLoadResponse] needle list: ", $(list));
    ...
    function getOwnedNeedles(root, retries=0) {
        if (retries == 0 || $(`#${root}`).length > 0)
            debug("[getOwnedNeedles] root: ", root, $(`#${root}`));

        if ($(`#${root}`).length == 0) {
            if (retries++ < 40) return setTimeout(getOwnedNeedles, 250, root, retries);
            return log("[getOwnedNeedles] timed out");
        }
        ...
        log("[getOwnedNeedles] needles: ", needles);
    }
