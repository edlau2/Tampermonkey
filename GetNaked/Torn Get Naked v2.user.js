// ==UserScript==
// @name         Torn Get Naked v2
// @namespace    http://tampermonkey.net/
// @version      0.2
// @description  Equip/Unequip items (armor and weapons) with one click
// @author       xedx [2100735]
...
(function($) {
    ...
    function addMainButtons() {
        ...
        var lastAction = GM_getValue('xedx-getnaked2-lastaction');
        if (lastAction == 'equip') {
            disableEquip();
    ...
    function createButtonDiv() {
        ...
        hdrDiv.appendChild(document.createTextNode('Get Naked v2!'));
    ...
    function checkLoad(callback, itemNode, name) { // itemNode passed in for debugging
        ...
        if (window.onLoad || itemNode.childElementCount > 1) {
            ...
            console.log('Get Naked v2 finished loading the "' + name +'" node, has ' + children + ' children.');
            callback(name);
        } else {
            setTimeout(function() {
        ...
        for (var i=0; i < len; i++) {
            ...
            if (name == 'primary-items' || name == 'secondary-items' || name == 'melee-items' ||
                ...
                if (children <= 1) {
                    ...
                    console.log('Get Naked v2 loaded ' + name +'. Has ' + children + ' children');
                    callback(name);
                }
            }
        }
    ...
    function saveWeaponMods(itemId) {
        ...
        var saved = GM_getValue('xedx-getnaked2-datasaved-' + itemId);
        if (saved == 'true') {
            return;
        }
    ...
    function realUnequip(node, name, id) {
        ...
        console.log('Get Naked v2 un-equipping ' + name);
        unequipBtn.click();
    }
    ...
    function unequipFunction() {
        GM_setValue('xedx-getnaked2-lastaction', 'unequip');
    ...
    function realEquip(node, name, id) {
        ...
        console.log('Get Naked v2 equipping ' + name + ', ID=' + id);
        equipBtn.click();
    }
    ...
    function equipFunction() {
        GM_setValue('xedx-getnaked2-lastaction', 'equip');
    ...
    function resetFunction() {
        GM_setValue('xedx-getnaked2-lastaction', 'reset');
        disableEquip(false);
        ...
        for (var i = 0; i < keyArray.length; i++) {
            key = keyArray[i];
            if(key.indexOf('xedx-getnaked2-datasaved') == -1){
                GM_deleteValue(key);
            }
        }

        GM_setValue('xedx-getnaked2-datasaved-primary-items', 'false');
        GM_setValue('xedx-getnaked2-datasaved-secondary-items', 'false');
        GM_setValue('xedx-getnaked2-datasaved-melee-items', 'false');
        GM_setValue('xedx-getnaked2-datasaved-temporary-items', 'false');
        GM_setValue('xedx-getnaked2-datasaved-armour-items', 'false');
    ...
    console.log("Torn Get Naked v2 script started!");
